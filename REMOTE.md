...
Use this when you want to connect to a CadSense server from another device such as a phone, tablet, or separate desktop app.
...
- HTTPS/WSS-compatible endpoints work from `https://app.cadsense.codes`, but are not made the default
  automatically.
...
If the copied link points directly at `http://192.168.x.y:3773`, open it from a client that can reach that LAN address. If it points at `https://app.cadsense.codes/pair?...`, the hosted web app will save the environment and connect directly to the backend URL in the link.
...
with the same server-side behavior as `cadsense serve --tailscale-serve`, then the server asks Tailscale
...
For `https://app.cadsense.codes`, prefer an HTTPS Tailnet or other HTTPS endpoint. A plain `http://100.x.y.z:3773` endpoint can still work from a desktop client or another browser page served over HTTP, but it will not work from the hosted HTTPS app because of browser mixed-content rules.
...
Run the server with `cadsense serve`.

```bash
npx cadsense serve --host "$(tailscale ip -4)"
```

`cadsense serve` starts the server without opening a browser and prints:
...
Use `cadsense serve --help` for the full flag reference. It supports the same general startup options as the normal server command, including an optional `cwd` argument.

For hosted web pairing over Tailscale HTTPS, opt in to Tailscale Serve:

```bash
npx cadsense serve --tailscale-serve
```
...
```bash
npx cadsense serve --tailscale-serve --tailscale-serve-port 8443
```

> Note
> The GUIs do not currently support adding projects on remote environments.
> For now, use `cadsense project ...` on the server machine instead.
> Full GUI support for remote project management is coming soon.

### Option 3: Desktop-Managed SSH Launch

Use this when you want the desktop app to start or reuse CadSense on another machine over SSH.
...
5. Confirm the launch. The desktop app probes the host, starts or reuses a remote CadSense server, opens a local port forward, and saves the environment.

After setup, the renderer connects to a local forwarded HTTP/WebSocket endpoint. The remote host still owns the actual CadSense server, projects, files, git state, terminals, and provider sessions.
...
The desktop SSH launcher connects with a non-interactive `sh` session, writes a small launcher script under `~/.cadsense/ssh-launch/<host-key>/`, starts or reuses a remote CadSense server, and forwards the remote loopback port back to your desktop.

The remote host must have a compatible Node.js runtime. CadSense uses the server package's `engines.node` requirement:

```text
^22.16 || ^23.11 || >=24.10
```

During SSH launch, CadSense first checks whether `node` is already available on `PATH`. If it is missing, the launcher tries common non-interactive shell locations and version-manager shims/activation hooks:
...
If launch fails with `node: command not found`, a port-scan failure, or a message that the remote Node version does not satisfy the required range, SSH into the host and check the same non-interactive shell path CadSense uses:
...
1. `cadsense serve` issues a one-time owner pairing token.
...
The hosted web app at `https://app.cadsense.codes` can save a remote backend in browser local storage from a URL like:

```text
https://app.cadsense.codes/pair?host=https://backend.example.com:3773#token=PAIRCODE
```
...
Hosted pairing does not proxy traffic through CadSense. The browser still connects directly to the backend URL in the pairing link.

## Managing Access Later

Use `cadsense auth` to manage access after the initial pairing flow.
...
Use `cadsense auth --help` and the nested subcommand help pages for the full reference.
...
- Use `cadsense auth` to revoke credentials or sessions you no longer trust.
