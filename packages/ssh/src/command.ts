...
import type { DesktopSshEnvironmentTarget, DesktopUpdateChannel } from "@cadsense/contracts";
...
const PUBLISHABLE_CADSENSE_VERSION_PATTERN = /^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/u;
...
export function resolveRemoteCadSenseCliPackageSpec(input: {
  readonly appVersion: string;
  readonly updateChannel: DesktopUpdateChannel;
  readonly isDevelopment?: boolean;
}): string {
  const appVersion = input.appVersion.trim();
  if (!input.isDevelopment && PUBLISHABLE_CADSENSE_VERSION_PATTERN.test(appVersion)) {
    return `cadsense@${appVersion}`;
  }

  if (input.isDevelopment) {
    return "cadsense@nightly";
  }

  return input.updateChannel === "nightly" ? "cadsense@nightly" : "cadsense@latest";
}
