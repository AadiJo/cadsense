...
import { extractJsonObject, fromLenientJson } from "@cadsense/shared/schemaJson";
...
export interface RemoteCadSenseRunnerOptions {
...
export interface SshEnvironmentManagerOptions {
  ...
  readonly resolveCliRunner?: Effect.Effect<RemoteCadSenseRunnerOptions>;
}
...
function sshRunnerLogFields(runner: RemoteCadSenseRunnerOptions | undefined) {
...
remote_node_satisfies_engine() {
  CADSENSE_NODE_ENGINE_RANGE=@@CADSENSE_NODE_ENGINE_RANGE@@
  if [ -z "$CADSENSE_NODE_ENGINE_RANGE" ]; then
    return 0
  fi
  node - "$CADSENSE_NODE_ENGINE_RANGE" <<'NODE'
@@CADSENSE_NODE_ENGINE_CHECK_SCRIPT@@
NODE
}
...
set -eu
@@CADSENSE_NODE_ENV_SCRIPT@@
ensure_remote_node_path || true
CADSENSE_NODE_SCRIPT_PATH=@@CADSENSE_NODE_SCRIPT_PATH@@
if [ -n "$CADSENSE_NODE_SCRIPT_PATH" ]; then
  ...
  exec node "$CADSENSE_NODE_SCRIPT_PATH" "$@"
fi
if command -v cadsense >/dev/null 2>&1; then
  exec cadsense "$@"
fi
if command -v npx >/dev/null 2>&1; then
  exec npx --yes @@CADSENSE_PACKAGE_SPEC@@ "$@"
fi
if command -v npm >/dev/null 2>&1; then
  exec npm exec --yes @@CADSENSE_PACKAGE_SPEC@@ -- "$@"
fi
printf 'Remote host is missing the cadsense CLI and could not install @@CADSENSE_PACKAGE_SPEC@@ because node/npm/npx are unavailable on PATH. Install Node or configure a supported version manager for non-interactive shells.\\n' >&2
exit 1
`;
...
@@CADSENSE_NODE_ENV_SCRIPT@@
STATE_KEY="$1"
STATE_DIR="$HOME/.cadsense/ssh-launch/$STATE_KEY"
DEFAULT_SERVER_HOME="$HOME/.cadsense"
DEFAULT_RUNTIME_FILE="$DEFAULT_SERVER_HOME/userdata/server-runtime.json"
PORT_FILE="$STATE_DIR/port"
PID_FILE="$STATE_DIR/pid"
MANAGED_FILE="$STATE_DIR/managed"
LOG_FILE="$STATE_DIR/server.log"
RUNNER_FILE="$STATE_DIR/run-cadsense.sh"
RUNNER_NEXT="$STATE_DIR/run-cadsense.next.$$"
mkdir -p "$STATE_DIR"
cleanup_runner_next() {
  rm -f "$RUNNER_NEXT"
}
...
pick_port() {
  node - "$PORT_FILE" "@@CADSENSE_DEFAULT_REMOTE_PORT@@" "@@CADSENSE_REMOTE_PORT_SCAN_WINDOW@@" <<'NODE'
@@CADSENSE_PICK_PORT_SCRIPT@@
NODE
}
wait_ready() {
  node - "$REMOTE_PORT" "$1" "@@CADSENSE_READY_PROBE_TIMEOUT_MS@@" <<'NODE'
@@CADSENSE_WAIT_READY_SCRIPT@@
NODE
}
wait_for_pid_exit() {
  PID_TO_WAIT="$1"
...
if [ -n "$DEFAULT_REMOTE_PORT" ]; then
  ...
  if wait_ready "@@CADSENSE_REUSE_READY_TIMEOUT_MS@@"; then
...
if [ "$REMOTE_MANAGED" = "external" ]; then
  if [ -z "$REMOTE_PORT" ] || ! wait_ready "@@CADSENSE_REUSE_READY_TIMEOUT_MS@@"; then
    REMOTE_PID=""
    REMOTE_PORT=""
    REMOTE_MANAGED=""
  fi
  ...
  if [ "$RUNNER_CHANGED" -eq 1 ]; then
    ...
    REMOTE_PORT=""
    REMOTE_MANAGED=""
  elif ! wait_ready "@@CADSENSE_REUSE_READY_TIMEOUT_MS@@"; then
    kill "$REMOTE_PID" 2>/dev/null || true
    wait_for_pid_exit "$REMOTE_PID"
    REMOTE_PID=""
    REMOTE_PORT=""
...
if [ -z "$REMOTE_PORT" ]; then
  ...
  nohup env CADSENSE_NO_BROWSER=1 "$RUNNER_FILE" serve --host 127.0.0.1 --port "$REMOTE_PORT" --base-dir "$DEFAULT_SERVER_HOME" >>"$LOG_FILE" 2>&1 < /dev/null &
  REMOTE_PID="$!"
  printf '%s\\n' "$REMOTE_PID" >"$PID_FILE"
  printf '%s\\n' "$REMOTE_PORT" >"$PORT_FILE"
  printf 'managed\\n' >"$MANAGED_FILE"
  if ! wait_ready "@@CADSENSE_READY_TIMEOUT_MS@@"; then
    printf 'Remote CadSense server did not become ready on 127.0.0.1:%s.\\n' "$REMOTE_PORT" >&2
...
STATE_DIR="$HOME/.cadsense/ssh-launch/@@CADSENSE_STATE_KEY@@"
DEFAULT_SERVER_HOME="$HOME/.cadsense"
RUNNER_FILE="$STATE_DIR/run-cadsense.sh"
mkdir -p "$STATE_DIR"
cat >"$RUNNER_FILE" <<'SH'
...
export function buildRemoteCadSenseRunnerScript(input?: RemoteCadSenseRunnerOptions): string {
  const packageSpec = shellSingleQuote(input?.packageSpec?.trim() || "cadsense@latest");
  const nodeScriptPath = input?.nodeScriptPath?.trim() || "";
  return stripTrailingNewlines(
    applyScriptPlaceholders(REMOTE_RUNNER_SCRIPT, {
      CADSENSE_PACKAGE_SPEC: packageSpec,
      CADSENSE_NODE_SCRIPT_PATH: shellSingleQuote(nodeScriptPath),
      CADSENSE_NODE_ENV_SCRIPT: buildRemoteNodeEnvScript(input),
    }),
  );
}

function buildRemoteNodeEnvScript(input?: RemoteCadSenseRunnerOptions): string {
  return stripTrailingNewlines(
    applyScriptPlaceholders(REMOTE_NODE_ENV_SCRIPT, {
      CADSENSE_NODE_ENGINE_RANGE: shellSingleQuote(input?.nodeEngineRange?.trim() || ""),
      CADSENSE_NODE_ENGINE_CHECK_SCRIPT: stripTrailingNewlines(buildRemoteNodeEngineCheckScript()),
    }),
  );
}

export function buildRemoteLaunchScript(input?: RemoteCadSenseRunnerOptions): string {
  return applyScriptPlaceholders(REMOTE_LAUNCH_SCRIPT, {
    CADSENSE_NODE_ENV_SCRIPT: buildRemoteNodeEnvScript(input),
    CADSENSE_RUNNER_SCRIPT: stripTrailingNewlines(buildRemoteCadSenseRunnerScript(input)),
    CADSENSE_PICK_PORT_SCRIPT: stripTrailingNewlines(REMOTE_PICK_PORT_SCRIPT),
    CADSENSE_WAIT_READY_SCRIPT: stripTrailingNewlines(REMOTE_WAIT_READY_SCRIPT),
    CADSENSE_DEFAULT_REMOTE_PORT: String(DEFAULT_REMOTE_PORT),
    CADSENSE_REMOTE_PORT_SCAN_WINDOW: String(REMOTE_PORT_SCAN_WINDOW),
    CADSENSE_READY_TIMEOUT_MS: String(REMOTE_READY_TIMEOUT_MS),
    CADSENSE_REUSE_READY_TIMEOUT_MS: String(REMOTE_REUSE_READY_TIMEOUT_MS),
    CADSENSE_READY_PROBE_TIMEOUT_MS: String(SSH_READY_PROBE_TIMEOUT_MS),
  });
}

export function buildRemotePairingScript(
  target: DesktopSshEnvironmentTarget,
  input?: RemoteCadSenseRunnerOptions,
): string {
  return applyScriptPlaceholders(REMOTE_PAIRING_SCRIPT, {
    CADSENSE_STATE_KEY: remoteStateKey(target),
    CADSENSE_RUNNER_SCRIPT: stripTrailingNewlines(buildRemoteCadSenseRunnerScript(input)),
  });
}
...
export const launchOrReuseRemoteServer = Effect.fn("ssh/tunnel.launchOrReuseRemoteServer")(
  function* (
    ...
    runner?: RemoteCadSenseRunnerOptions,
  ): Effect.fn.Return<
...
const makeSshEnvironmentManager = Effect.fn("ssh/tunnel.SshEnvironmentManager.make")(function* (
  ...
  const createTunnelEntry = Effect.fn("ssh/tunnel.ensureTunnelEntry.create")(function* (input: {
    readonly key: string;
    readonly resolvedTarget: DesktopSshEnvironmentTarget;
    readonly runner?: RemoteCadSenseRunnerOptions;
...
>()("@cadsense/ssh/SshEnvironmentManager") {
