...
import {
  ...
  buildRemoteCadSenseRunnerScript,
...
describe("ssh tunnel scripts", () => {
  it("builds the remote cadsense runner with npx and npm fallbacks", () => {
    const script = buildRemoteCadSenseRunnerScript({ nodeEngineRange: TEST_NODE_ENGINE_RANGE });

    assert.include(script, "CADSENSE_NODE_SCRIPT_PATH=''");
    assert.include(script, 'exec cadsense "$@"');
    assert.include(script, "exec npx --yes 'cadsense@latest' \"$@\"");
    assert.include(script, "exec npm exec --yes 'cadsense@latest' -- \"$@\"");
    assert.include(script, "could not install 'cadsense@latest'");
    assert.include(script, 'prepend_path_if_dir "$HOME/.local/bin"');
    assert.include(script, `CADSENSE_NODE_ENGINE_RANGE='${TEST_NODE_ENGINE_RANGE}'`);
    ...
    assert.include(script, 'for CADSENSE_NODE_BIN in "$NVM_DIR"/versions/node/*/bin');
    assert.notInclude(script, "ensure $NVM_DIR/nvm.sh is available");
  });

  it("does not hard-code a remote node engine range", () => {
    const script = buildRemoteCadSenseRunnerScript();

    assert.include(script, "CADSENSE_NODE_ENGINE_RANGE=''");
    assert.notInclude(script, TEST_NODE_ENGINE_RANGE);
  });

  it("shell-quotes package specs in the remote cadsense runner", () => {
    const script = buildRemoteCadSenseRunnerScript({
      packageSpec: "cadsense@nightly; touch /tmp/cadsense-owned",
    });

    assert.include(script, "exec npx --yes 'cadsense@nightly; touch /tmp/cadsense-owned' \"$@\"");
    assert.include(
      script,
      "exec npm exec --yes 'cadsense@nightly; touch /tmp/cadsense-owned' -- \"$@\"",
    );
    assert.notInclude(script, "exec npx --yes cadsense@nightly; touch /tmp/cadsense-owned");
  });

  it("builds the remote cadsense runner with a node script override", () => {
    const script = buildRemoteCadSenseRunnerScript({
    ...
    assert.include(
      script,
      "CADSENSE_NODE_SCRIPT_PATH='/Users/julius/Development/Work/codething-mvp/apps/server/dist/bin.mjs'",
    );
    assert.include(script, 'exec node "$CADSENSE_NODE_SCRIPT_PATH" "$@"');
  });

  it("uses the remote cadsense runner for launch and pairing scripts", () => {
    const target = {
      alias: "devbox",
    ...
    assert.include(
      buildRemoteLaunchScript({ nodeEngineRange: TEST_NODE_ENGINE_RANGE }),
      `CADSENSE_NODE_ENGINE_RANGE='${TEST_NODE_ENGINE_RANGE}'`,
    );
    assert.include(
      buildRemoteLaunchScript({ nodeEngineRange: TEST_NODE_ENGINE_RANGE }),
    ...
    assert.include(buildRemoteLaunchScript(), "Remote CadSense server did not become ready");
    assert.include(
      buildRemoteLaunchScript({ packageSpec: "cadsense@nightly" }),
      "cadsense@nightly",
    );
    assert.include(
    ...
    assert.include(
      buildRemotePairingScript(target, { packageSpec: "cadsense@nightly" }),
      "cadsense@nightly",
    );
    assert.include(
