...
import {
  ...
  resolveRemoteCadSenseCliPackageSpec,
  runSshCommand,
} from "./command.ts";
...
describe("ssh command", () => {
  ...
  it.effect("resolves the remote cadsense package spec from the desktop release channel", () =>
    Effect.sync(() => {
      assert.equal(
        resolveRemoteCadSenseCliPackageSpec({
          appVersion: "0.0.17",
      ...
      assert.equal(
        ...
        "cadsense@0.0.17-nightly.20260415.44",
      );
      assert.equal(
        resolveRemoteCadSenseCliPackageSpec({
