...
describe("ssh auth", () => {
  ...
  it.effect("creates askpass env for cached password prompts", () =>
    Effect.gen(function* () {
      ...
      const directory = yield* fs.makeTempDirectoryScoped({ prefix: "cadsense-ssh-askpass-test-" });
      ...
      assert.equal(env.CADSENSE_SSH_AUTH_SECRET, "super-secret");
      assert.equal(env.DISPLAY, "cadsense");
  ...
  it.effect("builds a windows askpass launcher pair", () =>
    Effect.gen(function* () {
      const descriptor = yield* buildSshAskpassHelperDescriptor({
        directory: "C:\\temp\\cadsense-ssh-askpass",
        platform: "win32",
      }).pipe(Effect.provide(NodeServices.layer));

      assert.equal(descriptor.launcherPath, "C:\\temp\\cadsense-ssh-askpass\\ssh-askpass.cmd");
      assert.deepEqual(
