...
Define the long-term server auth architecture for CadSense before first-class remote environments ship.
...
Every privileged surface of the CadSense server must go through the same auth policy engine:
...
Participants:
  ...
  CadSenseServer      = local backend child process
...
DesktopMain -> CadSenseServer : spawn server (--bootstrap-fd ...)
DesktopMain -> CadSenseServer : send desktop bootstrap envelope
note over CadSenseServer : policy = DesktopManagedLocalPolicy
note over CadSenseServer : allowed pairing = desktop-bootstrap only

Frontend -> DesktopMain : request local bootstrap grant
DesktopMain --> Frontend : short-lived desktop bootstrap grant

Frontend -> CadSenseServer : POST /api/auth/bootstrap
CadSenseServer -> CadSenseServer : validate desktop bootstrap grant
CadSenseServer -> CadSenseServer : create browser session
CadSenseServer --> Frontend : Set-Cookie: session=...

Frontend -> CadSenseServer : GET /ws + authenticated cookie
CadSenseServer -> CadSenseServer : validate cookie session
CadSenseServer --> Frontend : websocket accepted
```
...
```text
Participants:
  UserShell     = npx cadsense launcher
  CadSenseServer      = standalone local server
  Browser       = browser tab

UserShell -> CadSenseServer : start server
CadSenseServer -> CadSenseServer : getOrCreate("server-signing-key")
note over CadSenseServer : policy = LoopbackBrowserPolicy

UserShell -> CadSenseServer : issue one-time pairing token
CadSenseServer --> UserShell : pairing URL or pairing token

UserShell --> Browser : open /pair?token=...

Browser -> CadSenseServer : GET /pair?token=...
CadSenseServer -> CadSenseServer : validate one-time token
CadSenseServer -> CadSenseServer : create browser session
CadSenseServer --> Browser : Set-Cookie: session=...
CadSenseServer --> Browser : redirect to app

Browser -> CadSenseServer : GET /ws + authenticated cookie
CadSenseServer --> Browser : websocket accepted
```
...
Participants:
  ...
  CadSenseServer      = CadSense server
  PhoneBrowser  = mobile browser

DesktopUser -> DesktopMain : enable remote access via tunnel
DesktopMain -> CadSenseServer : switch policy to RemoteReachablePolicy
DesktopMain -> Tunnel : publish local CadSense endpoint
Tunnel --> DesktopMain : public https/wss URL

DesktopMain -> CadSenseServer : issue one-time pairing token
CadSenseServer --> DesktopMain : pairing token
...
Tunnel -> CadSenseServer : forward request
CadSenseServer -> CadSenseServer : validate one-time token
CadSenseServer -> CadSenseServer : create mobile browser session
CadSenseServer --> PhoneBrowser : Set-Cookie: session=...
CadSenseServer --> PhoneBrowser : redirect to app

PhoneBrowser -> Tunnel : GET /ws + authenticated cookie
Tunnel -> CadSenseServer : forward websocket upgrade
CadSenseServer --> PhoneBrowser : websocket accepted
```
...
DesktopUser -> CadSenseServer : enable private-network access
CadSenseServer -> CadSenseServer : switch policy to RemoteReachablePolicy
DesktopUser -> CadSenseServer : issue one-time pairing token
CadSenseServer --> DesktopUser : pairing URL / QR

DesktopUser -> PhoneBrowser : open private-network URL
PhoneBrowser -> PrivateNet : GET http(s)://private-host/pair?token=...
PrivateNet -> CadSenseServer : route request
CadSenseServer -> CadSenseServer : validate one-time token
CadSenseServer -> CadSenseServer : create mobile browser session
CadSenseServer --> PhoneBrowser : Set-Cookie: session=...
CadSenseServer --> PhoneBrowser : redirect to app
...
PrivateNet -> CadSenseServer : websocket upgrade
CadSenseServer --> PhoneBrowser : websocket accepted
```
...
Participants:
  ...
  RemoteCadSense      = remote CadSense server
  Frontend      = desktop renderer

DesktopUser -> DesktopMain : add SSH host
DesktopMain -> SSH : connect to remote host
SSH -> RemoteHost : probe environment / verify cadsense availability
DesktopMain -> SSH : run remote launch command
SSH -> RemoteHost : cadsense remote launch --json
RemoteHost -> RemoteCadSense : start or reuse server
RemoteCadSense --> RemoteHost : port + environment metadata
...
note over RemoteCadSense : policy = RemoteReachablePolicy
note over DesktopMain,RemoteCadSense : desktop may use a trusted bootstrap flow here

Frontend -> DesktopMain : request bootstrap for selected environment
DesktopMain --> Frontend : short-lived bootstrap grant

Frontend -> RemoteCadSense : POST /api/auth/bootstrap via forwarded port
RemoteCadSense -> RemoteCadSense : validate bootstrap grant
RemoteCadSense -> RemoteCadSense : create browser session
RemoteCadSense --> Frontend : Set-Cookie: session=...

Frontend -> RemoteCadSense : GET /ws + authenticated cookie
RemoteCadSense --> Frontend : websocket accepted
```
