...
- users can expose a backend through LAN, their own Tailscale, MagicDNS, a manual HTTPS endpoint, or later CadSense Tunnel
- users can generate a hosted pairing link for `app.cadsense.codes`
...
This plan intentionally leaves the paid CadSense cloud tunnel fabric out of scope. It defines the OSS foundation that CadSense Tunnel should later plug into.
...
- `https://app.cadsense.codes` cannot reliably call `http://...` or `ws://...` endpoints because browsers block mixed content.
...
- Generate `app.cadsense.codes` links only from endpoints that are likely hosted-browser compatible, or show a warning with an explicit fallback.
...
```text
https://app.cadsense.codes/pair?host=<encoded endpoint httpBaseUrl>#token=<one-time token>
```
...
- HTTP private-network endpoints are marked incompatible with `app.cadsense.codes`.
...
- `app.cadsense.codes` works as a real client shell.
...
- `app.cadsense.codes` can pair a reachable HTTPS backend and reconnect after reload.
- A saved environment can be used without any backend at `app.cadsense.codes`.
...
## Phase 4: Future CadSense Tunnel Provider
...
- create endpoint with `provider: "cadsense-tunnel"`
- surface tunnel status
- provide stable HTTPS URL
- use existing backend pairing/session auth
- never bypass server auth

The tunnel fabric can later be Pipenet-derived, Tailscale-derived, or another reverse tunnel implementation. The rest of CadSense should only see an `AdvertisedEndpoint`.
