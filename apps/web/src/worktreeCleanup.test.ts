import { EnvironmentId, ProjectId, ProviderInstanceId, ThreadId } from "@cadsense/contracts";
...
describe("formatWorktreePathForDisplay", () => {
  it("shows only the last path segment for unix-like paths", () => {
    const result = formatWorktreePathForDisplay(
      "/Users/julius/.cadsense/worktrees/cadsense-mvp/cadsense-4e609bb8",
    );
    expect(result).toBe("cadsense-4e609bb8");
  });

  it("normalizes windows separators before selecting the final segment", () => {
    const result = formatWorktreePathForDisplay(
      "C:\\Users\\julius\\.cadsense\\worktrees\\cadsense-mvp\\cadsense-4e609bb8",
    );
    expect(result).toBe("cadsense-4e609bb8");
  });

  it("uses the final segment even when outside ~/.cadsense/worktrees", () => {
