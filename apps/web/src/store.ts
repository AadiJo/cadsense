...
import { isProviderDriverKind, ProviderDriverKind } from "@cadsense/contracts";
import type { ThreadId, TurnId } from "@cadsense/contracts";
import * as Schema from "effect/Schema";
import { resolveModelSlugForProvider } from "@cadsense/shared/model";
...
export interface EnvironmentState {
  ...
  reviewIdsByThreadId?: Record<ThreadId, CadReviewId[]>;
  reviewByThreadId?: Record<ThreadId, Record<CadReviewId, CadReviewReport>>;
...
function mapProject(
  ...
  return {
    ...
    externalContext: project.externalContext ?? null,
...
function mapThread(thread: OrchestrationThread, environmentId: EnvironmentId): Thread {
  return {
    ...
    reviews: thread.reviews?.map((review) => ({ ...review })) ?? [],
...
function buildReviewSlice(thread: Thread): {
  ids: CadReviewId[];
  byId: Record<CadReviewId, CadReviewReport>;
} {
  const reviews = thread.reviews ?? [];
  return {
    ids: reviews.map((review) => review.id),
    byId: Object.fromEntries(reviews.map((review) => [review.id, review] as const)) as Record<
      CadReviewId,
      CadReviewReport
    >,
  };
...
function writeThreadState(
  ...
  if (previousThread?.reviews !== nextThread.reviews) {
    const nextReviewSlice = buildReviewSlice(nextThread);
    nextState = {
      ...nextState,
      reviewIdsByThreadId: {
        ...nextState.reviewIdsByThreadId,
        [nextThread.id]: nextReviewSlice.ids,
      },
      reviewByThreadId: {
        ...nextState.reviewByThreadId,
        [nextThread.id]: nextReviewSlice.byId,
      },
    };
  }
...
function removeThreadState(state: EnvironmentState, threadId: ThreadId): EnvironmentState {
  ...
  const { [threadId]: _removedReviewIds, ...reviewIdsByThreadId } = state.reviewIdsByThreadId ?? {};
  const { [threadId]: _removedReviews, ...reviewByThreadId } = state.reviewByThreadId ?? {};
