...
function onshapeProjectInputPlaceholder(flow: AddProjectOnshapeFlow | null): string | null {
  if (!flow) return null;
  if (flow.step === "connection") {
    return "Set up an Onshape API key in Settings > Connections";
  }
  return "Paste an Onshape document or element URL";
}

function onshapeIcon(className: string): ReactNode {
  return <img src="/onshape.svg" alt="" className={cn(className, "rounded-sm object-contain")} />;
}

function onshapeShadowWorkspaceRoot(entity: OnshapeEntity): string {
  const slug = entity.entityId.replace(/[^a-z0-9_-]+/gi, "-").slice(0, 96);
  return `~/.cadsense/chats/${slug}`;
}

function selectImportedOnshapeEntity(entities: ReadonlyArray<OnshapeEntity>): OnshapeEntity | null {
  return (
    entities.find((entity) => entity.partId !== null) ??
    entities.find((entity) => entity.elementId !== null) ??
    entities[0] ??
    null
  );
}
...
function OpenCommandPaletteDialog() {
  ...
  const isOnshapeFlow = addProjectOnshapeFlow !== null;
  const isBrowsing = !isOnshapeFlow && isFilesystemBrowseQuery(query, browseEnvironmentPlatform);
  ...
  const startAddProjectOnshape = useCallback((environmentId: EnvironmentId): void => {
    ...
    setAddProjectOnshapeFlow(null);
    pushPaletteView({
      addonIcon: onshapeIcon(ADDON_ICON_CLASS),
      groups: [],
      initialQuery: "",
    });
    ...
    void api.onshape
      ...
      .then((result) => {
        ...
        setAddProjectOnshapeFlow(
          connection
            ? { step: "url", environmentId, connection }
            : { step: "connection", environmentId },
        );
      })
      .catch((error) => {
        if (addProjectOnshapeRequestIdRef.current !== requestId) return;
        toastManager.add(
          stackedThreadToast({
            type: "error",
            title: "Unable to load Onshape connections",
  ...
  const buildAddProjectSourceGroups = useCallback(
    (environmentId: EnvironmentId): CommandPaletteView["groups"] => {
      const sourceItems: Array<CommandPaletteActionItem | CommandPaletteSubmenuItem> = [
        ...
        {
          kind: "action",
          value: `action:add-project:${environmentId}:onshape`,
          searchTerms: ["onshape", "cad", "document", "part", "folder", "workspace"],
          title: "Onshape document or folder",
          description: "Index an Onshape URL and create a local chat workspace",
          icon: onshapeIcon(ITEM_ICON_CLASS),
          keepOpen: true,
          run: async () => {
            startAddProjectOnshape(environmentId);
          },
        },
      ];
    ...
    [startAddProjectBrowse, startAddProjectOnshape],
  );
  ...
  actionItems.push({
    ...
    searchTerms: ["add project", "folder", "directory", "browse", "url", "environment"],
    title: "Add project",
    icon: <FolderPlusIcon className={ITEM_ICON_CLASS} />,
    keepOpen: true,
    run: async () => {
  ...
  async function submitAddProjectOnshapeFlow(): Promise<void> {
    if (!addProjectOnshapeFlow || isOnshapePending) {
      return;
    }

    const api = readEnvironmentApi(addProjectOnshapeFlow.environmentId);
    if (!api) {
      toastManager.add(
        stackedThreadToast({
          type: "error",
          title: "Unable to add Onshape workspace",
    ...
    if (addProjectOnshapeFlow.step === "connection") {
    ...
    setOpen(false);
    try {
      const result = await api.onshape.importUrl({
        connectionId: addProjectOnshapeFlow.connection.connectionId,
        url,
        includeParts: false,
      });
      const entity = selectImportedOnshapeEntity(result.entities);
      if (!entity) {
        throw new Error("Onshape index did not return any entities.");
      }

      await createOnshapeProject(
        addProjectOnshapeFlow.environmentId,
        addProjectOnshapeFlow.connection,
        entity,
      );
    } catch (error) {
      toastManager.add(
        stackedThreadToast({
          type: "error",
          title: "Failed to add Onshape workspace",
          description: error instanceof Error ? error.message : "An error occurred.",
        }),
      );
    } finally {
  ...
  const createOnshapeProject = useCallback(
    async (
      environmentId: EnvironmentId,
      connection: OnshapeConnection,
      entity: OnshapeEntity,
    ): Promise<void> => {
      const api = readEnvironmentApi(environmentId);
      if (!api) {
      ...
      const projectId = newProjectId();
      const workspaceRoot = onshapeShadowWorkspaceRoot(entity);
      const breadcrumb = entity.breadcrumb.length > 0 ? entity.breadcrumb : [entity.name];
      await api.orchestration.dispatchCommand({
        type: "project.create",
        commandId: newCommandId(),
        projectId,
        title: breadcrumb.at(-1) ?? entity.name,
        workspaceRoot,
        createWorkspaceRootIfMissing: true,
        externalContext: {
          provider: "onshape",
          onshape: {
            ...
            reference: {
              ...
              ...(entity.wvmKind ? { wvmKind: entity.wvmKind } : {}),
              ...(entity.wvmId ? { wvmId: entity.wvmId } : {}),
              ...(entity.elementId ? { elementId: entity.elementId } : {}),
              ...(entity.partId ? { partId: entity.partId } : {}),
      ...
      await waitForProjectedProject({ environmentId, projectId });
      await handleNewThread(scopeProjectRef(environmentId, projectId), {
        envMode: settings.defaultThreadEnvMode,
      }).catch(() => undefined);
    },
    [handleNewThread, settings.defaultThreadEnvMode],
  );
  ...
  const onshapeSearchEnabled =
    addProjectOnshapeFlow?.step === "url" &&
    deferredQuery.trim().length > 0 &&
    !/^https?:\/\//iu.test(deferredQuery.trim());
  const onshapeSearchQuery = useQuery({
    queryKey: [
      "onshape-search",
      addProjectOnshapeFlow?.step === "url" ? addProjectOnshapeFlow.connection.connectionId : null,
      deferredQuery.trim(),
    ],
    enabled: onshapeSearchEnabled,
    queryFn: async () => {
      if (addProjectOnshapeFlow?.step !== "url") {
        return { entities: [] };
      }
      const api = readEnvironmentApi(addProjectOnshapeFlow.environmentId);
      if (!api) {
      ...
      return api.onshape.searchIndex({
        connectionId: addProjectOnshapeFlow.connection.connectionId,
  ...
  const onshapeSearchGroups = useMemo<CommandPaletteView["groups"]>(() => {
    if (addProjectOnshapeFlow?.step === "connection") {
      return [
        {
          value: "onshape-setup",
          ...
          items: [
            {
              kind: "action",
              value: "onshape:open-connections-settings",
              searchTerms: ["settings", "connections", "api", "key", "secret", "setup"],
              title: "Set up Onshape in Connections",
              description: "Add an Onshape API key before importing documents",
              icon: onshapeIcon(ITEM_ICON_CLASS),
              keepOpen: false,
              run: async () => {
                openConnectionsSettings();
    ...
    const entities = onshapeSearchQuery.data?.entities ?? [];
    return [
      {
        value: "onshape-actions",
        label: "Onshape",
        items: [
          {
            kind: "action",
            value: "onshape:refresh",
            searchTerms: ["refresh", "index", "sync", "update"],
            title: "Refresh Onshape index",
            description: "Refresh indexed metadata for this connection",
            icon: onshapeIcon(ITEM_ICON_CLASS),
            keepOpen: true,
            run: async () => {
              const api = readEnvironmentApi(addProjectOnshapeFlow.environmentId);
              ...
              setIsOnshapePending(true);
              try {
                await api.onshape.refreshIndex({
                  connectionId: addProjectOnshapeFlow.connection.connectionId,
                });
                await onshapeSearchQuery.refetch();
              } catch (error) {
                toastManager.add(
                  stackedThreadToast({
                    type: "error",
                    title: "Failed to refresh Onshape index",
                    description: error instanceof Error ? error.message : "An error occurred.",
                  }),
                );
              } finally {
      ...
      {
        value: "onshape-index",
        label: "Indexed Onshape",
        items: entities.map(
          (entity): CommandPaletteActionItem => ({
            kind: "action",
            value: `onshape:${entity.entityId}`,
            searchTerms: [entity.name, entity.kind, ...entity.breadcrumb],
            title: entity.name,
            description: `${entity.kind} • ${entity.breadcrumb.join(" > ")}`,
            icon: onshapeIcon(ITEM_ICON_CLASS),
            keepOpen: false,
            run: async () => {
              try {
                ...
                toastManager.add(
                  stackedThreadToast({
                    type: "error",
                    title: "Failed to open Onshape chat",
                    description: error instanceof Error ? error.message : "An error occurred.",
                  }),
                );
              }
  ...
  }, [addProjectOnshapeFlow, createOnshapeProject, onshapeSearchQuery, openConnectionsSettings]);

  let displayedGroups: CommandPaletteView["groups"] = filteredGroups;
  if (addProjectOnshapeFlow) {
    displayedGroups = onshapeSearchGroups;
  } else if (isBrowsing) {
    displayedGroups = relativePathNeedsActiveProject ? [] : browseGroups;
  }

  const inputPlaceholder =
    onshapeProjectInputPlaceholder(addProjectOnshapeFlow) ??
  ...
  const canSubmitOnshapeFlow =
    addProjectOnshapeFlow !== null && query.trim().length > 0 && !isOnshapePending;
  ...
  function handleKeyDown(event: KeyboardEvent<HTMLInputElement>): void {
    if (addProjectOnshapeFlow && event.key === "Enter") {
      event.preventDefault();
      void submitAddProjectOnshapeFlow();
      return;
    }
  ...
  return (
    ...
    >
      <Command
        key={`${viewStack.length}-${browseGeneration}-${isBrowsing}-${addProjectOnshapeFlow?.step ?? "none"}`}
        aria-label="Command palette"
        autoHighlight={isBrowsing || isOnshapeFlow ? false : "always"}
        mode="none"
        ...
        <div className="relative">
          <CommandInput
            ...
            {...(isSubmenu
              ...
              : addProjectOnshapeFlow
                ? {
                    startAddon: onshapeIcon(ADDON_ICON_CLASS),
                  }
          ...
          {addProjectOnshapeFlow ? (
            ...
            >
              <span>{isOnshapePending ? "Working" : "Continue"}</span>
        ...
        <CommandPanel className="max-h-[min(28rem,70vh)]">
          <CommandPaletteResults
            ...
            {...(addProjectOnshapeFlow
              ? {
                  emptyStateMessage:
                    addProjectOnshapeFlow.step === "connection"
                      ? "Enter Onshape credentials separated by pipes, then press Enter."
                      : "Paste an Onshape document or element URL and press Enter.",
                }
              : relativePathNeedsActiveProject
        ...
        <CommandFooter className="gap-3 max-sm:flex-col max-sm:items-start">
          <div className="flex items-center gap-3">
            ...
            {addProjectOnshapeFlow ? (
              <KbdGroup className="items-center gap-1.5">
                <Kbd>Enter</Kbd>
                <span className={cn("text-muted-foreground/80")}>Continue</span>
              </KbdGroup>
