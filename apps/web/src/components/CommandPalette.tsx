...
function OpenCommandPaletteDialog() {
  ...
  return (
    ...
    >
      <Command
        key={`${viewStack.length}-${browseGeneration}-${isBrowsing}-${addProjectCloneFlow?.step ?? "none"}-${addProjectOnshapeFlow?.step ?? "none"}`}
        aria-label="Command palette"
        autoHighlight={isBrowsing || isRemoteProjectCloneFlow || isOnshapeFlow ? false : "always"}
        mode="none"
