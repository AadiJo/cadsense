...
import { scopedThreadKey, scopeProjectRef, scopeThreadRef } from "@cadsense/client-runtime";
import {
  applyClaudePromptEffortPrefix,
  createModelSelection,
  resolvePromptInjectedEffort,
} from "@cadsense/shared/model";
import { projectScriptCwd } from "@cadsense/shared/projectScripts";
...
export default function ChatView(props: ChatViewProps) {
  ...
  const focusComposer = useCallback(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- stable ref to imperative composer handle
  }, []);
  ...
  const syncOnshapeProject = useCallback(async () => {
    if (!activeProject || activeProject.externalContext?.provider !== "onshape") return;
    ...
    const syncingProjectId = activeProject.id;
    setOnshapeSyncingProjectIds((prev) => {
      const next = new Set(prev);
      next.add(syncingProjectId);
      return next;
    });
    try {
      const result = await api.onshape.syncProject({
        cwd: activeProject.cwd,
        context: activeProject.externalContext.onshape,
      });
      const { lastSyncError: _lastSyncError, ...syncedContextBase } =
      ...
      await api.orchestration.dispatchCommand({
        ...
        externalContext: {
          ...
          onshape: {
            ...
            lastSyncedRelativePath: result.relativePath,
            lastSyncedFormat: result.format,
          },
        },
      });
      toastManager.add(
        stackedThreadToast({
          type: "success",
          title: "Onshape synced",
          description: `Saved ${result.relativePath}`,
        }),
      );
    } catch (error) {
      const message = formatCaughtErrorMessage(error, "An unexpected error occurred.");
      await api.orchestration.dispatchCommand({
        type: "project.meta.update",
        commandId: newCommandId(),
        projectId: activeProject.id,
  ...
  const onSelectActivePendingUserInputOption = useCallback(
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- composerRef is a stable ref to imperative composer
  ...
  const onImplementPlanInNewThread = useCallback(async () => {
    ...
    await api.orchestration
      .dispatchCommand({
        ...
        ...(activeThread.externalContext ? { externalContext: activeThread.externalContext } : {}),
