...
import { createModelCapabilities, createModelSelection } from "@cadsense/shared/model";
...
describe("ChatView timeline estimator parity (full app)", () => {
  ...
  it("falls back to the first installed editor when the stored favorite is unavailable", async () => {
    localStorage.setItem("cadsense:last-editor", JSON.stringify("vscodium"));
    setDraftThreadWithoutWorktree();
      ...
      await vi.waitFor(
        () => {
          ...
          expect(openRequest).toMatchObject({
            ...
            env: {
              CADSENSE_PROJECT_ROOT: "/repo/project",
              CADSENSE_WORKTREE_PATH: "/repo/worktrees/feature-draft",
            },
          });
        },
        { timeout: 8_000, interval: 16 },
        ...
        if (body._tag === WS_METHODS.gitResolvePullRequest) {
          return {
            pullRequest: {
              ...
              url: "https://github.com/pingdotgg/cadsense/pull/1359",
              baseBranch: "main",
  ...
  it("shows the confirm archive action after clicking the archive button", async () => {
    ...
    try {
      ...
      localStorage.removeItem("cadsense:client-settings:v1");
      await mounted.cleanup();
    }
  });
        ...
        threads: createSnapshotForTargetUser({
          ...
          thread.id === THREAD_ID
            ? Object.assign({}, thread, {
                ...
                worktreePath: "/repo/.cadsense/worktrees/existing",
              })
            : thread,
        ),
      },
  ...
  it("keeps the wide desktop follow-up layout expanded when the footer still fits", async () => {
    const mounted = await mountChatView({
      ...
      snapshot: createSnapshotWithPlanFollowUpPrompt({
        ...
        planMarkdown:
          "# Imaginary Long-Range Plan: CadSense Adaptive Orchestration and Safe-Delay Execution Initiative",
      }),
    });
