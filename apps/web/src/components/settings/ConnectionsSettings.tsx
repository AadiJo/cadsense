...
const showHiddenSettingsSections = () => false;
...
function OnshapeConnectionListRow({ connection, onEdit }: OnshapeConnectionListRowProps) {
  return (
    <div className={ITEM_ROW_CLASSNAME}>
      <div className={ITEM_ROW_INNER_CLASSNAME}>
        <div className="min-w-0 flex-1 space-y-1">
        ...
        <div className="flex w-full shrink-0 items-center gap-2 sm:w-auto sm:justify-end">
          <Button size="xs" variant="outline" onClick={() => onEdit(connection)}>
            Update key
          </Button>
        </div>
      </div>
...
export function ConnectionsSettings() {
  ...
  const loadOnshapeConnections = useCallback(async () => {
    ...
    try {
      ...
      setOnshapeConnectionError(message);
    } finally {
  ...
  const handleSaveOnshapeConnection = useCallback(async () => {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : "Failed to save Onshape connection.";
      setOnshapeConnectionError(message);
    } finally {
  ...
  const renderOnshapeConnectionsSection = () => (
    <SettingsSection
      title="Onshape"
      headerAction={
          ...
          >
            ...
            <DialogPopup className="max-w-md">
              <DialogHeader>
                ...
                <DialogDescription>
                  API secrets are stored in the local backend secret store. Only redacted connection
                  metadata is shown in the app.
                </DialogDescription>
              </DialogHeader>
              <DialogPanel className="space-y-4">
                <label className="block">
                ...
                <label className="block">
                  ...
                  <Input
                    ...
                    placeholder={
                      onshapeEditingConnection ? "Enter a new secret key" : "Onshape API secret key"
                    }
                    disabled={isSavingOnshapeConnection}
                    spellCheck={false}
                  />
    ...
    >
      ...
      {onshapeConnections.length === 0 && !isLoadingOnshapeConnections ? (
        <div className={ITEM_ROW_CLASSNAME}>
          <p className="text-xs text-muted-foreground">
            No Onshape API keys configured. Add a connection here, then import Onshape document URLs
  ...
  return (
    <SettingsPageContainer>
      {showHiddenSettingsSections() && canManageLocalBackend ? (
        <>
          ...
          >
            <AlertDialogPopup>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  {pendingDesktopServerExposureMode === "network-accessible"
                    ? "CadSense will restart to expose this environment over the network."
                    : "CadSense will restart and limit this environment back to this machine."}
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogClose
          ...
          >
            <AlertDialogPopup>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  CadSense will restart the local backend without Tailscale Serve.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogClose
          ...
          >
            <DialogPopup className="max-w-md">
              <DialogHeader>
                ...
                <DialogDescription>
                  CadSense will restart the local backend with Tailscale Serve enabled and ask
