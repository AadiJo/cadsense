...
describe("GeneralSettingsPanel observability", () => {
  ...
  it("shows a disabled network access toggle with guidance in desktop builds", async () => {
    ...
    await expect
      .element(page.getByText("CadSense will restart to expose this environment over the network."))
      .toBeInTheDocument();
  ...
  it("opens the logs folder in the preferred editor", async () => {
    ...
    window.nativeApi = {
      ...
      server: {
        ...
        getTraceDiagnostics: vi.fn().mockResolvedValue({
          traceFilePath: "/repo/project/.cadsense/traces.jsonl",
          scannedFilePaths: ["/repo/project/.cadsense/traces.jsonl"],
          readAt: makeUtc("2036-04-07T00:00:00.000Z"),
          recordCount: 0,
          parseErrorCount: 0,
    ...
    expect(openInEditor).toHaveBeenCalledWith("/repo/project/.cadsense/logs", "cursor");
  });
