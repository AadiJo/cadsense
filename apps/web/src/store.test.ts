...
describe("incremental orchestration updates", () => {
  ...
  it("preserves Onshape externalContext on project.created", () => {
    const projectId = ProjectId.make("project-onshape");
    ...
    const externalContext = {
      ...
      onshape: {
        ...
        name: "Bracket",
        breadcrumb: ["Bracket"],
        reference: { baseUrl: "https://cad.onshape.com", documentId: "doc-1", url: "https://x" },
      },
    };
    const next = applyOrchestrationEvent(
      state,
      makeEvent("project.created", {
        projectId,
        title: "Bracket",
        workspaceRoot: "~/.cadsense/chats/onshape-slug",
        externalContext,
        defaultModelSelection: {
          instanceId: ProviderInstanceId.make("codex"),
          model: DEFAULT_MODEL,
    ...
    expect(localEnvironmentStateOf(next).projectById[projectId]?.externalContext).toEqual(
      externalContext,
    );
  });
  ...
  it("preserves Onshape externalContext on thread.created", () => {
    const projectId = ProjectId.make("project-onshape-thread");
    const threadId = ThreadId.make("thread-onshape");
    const state = applyOrchestrationEvent(
      makeEmptyState({}),
      makeEvent("project.created", {
        projectId,
        title: "Doc",
        workspaceRoot: "~/.cadsense/chats/x",
        defaultModelSelection: {
          instanceId: ProviderInstanceId.make("codex"),
          model: DEFAULT_MODEL,
        },
    ...
    const externalContext = {
      ...
      onshape: {
        ...
        name: "Doc",
        breadcrumb: ["Doc"],
        reference: { baseUrl: "https://cad.onshape.com" },
      },
    };
    const next = applyOrchestrationEvent(
      state,
    ...
    expect(
      selectThreadByRef(next, scopeThreadRef(localEnvironmentId, threadId))?.externalContext,
    ).toEqual(externalContext);
  });
