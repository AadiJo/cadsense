...
/**
 * Chromium flags prepended to the Electron argv so they apply before JS loads.
 * WSLg often reports a blocklisted GPU for WebGL1; in-process `appendSwitch` alone
 * can be too late for the GPU sandbox path.
 */
export function electronGpuBootstrapArgv() {
  const argv = ["--ignore-gpu-blocklist"];
  if (process.env.WSL_DISTRO_NAME || process.env.WSL_INTEROP) {
    argv.push("--disable-gpu-sandbox");
  }
  return argv;
}
