      ...
      DesktopAppIdentity.layer.pipe(
        Layer.provideMerge(
          FileSystem.layerNoop({
            exists: (path) =>
              Effect.succeed(input.legacyPathExists === true && path.includes("CadSense (Alpha)")),
            readFileString: () =>
  ...
  it.effect("keeps using the legacy userData path when it already exists", () =>
    withIdentity(
      Effect.gen(function* () {
        ...
        assert.equal(
          normalizePathForAssert(userDataPath),
          "/Users/alice/Library/Application Support/CadSense (Alpha)",
        );
      }),
