    ...
    Effect.gen(function* () {
      ...
      yield* runCommand(
        ChildProcess.make(process.execPath, ["--run", "build:bundle"], {
          ...
          stderr: "inherit",
          // Never use `shell: true` here: on Windows, `process.execPath` is often under
          // `C:\Program Files\...` and Node's shell concatenation splits on spaces (DEP0190).
          shell: false,
        }),
      );
...
const cli = Command.make("cli").pipe(
  Command.withDescription("CadSense server build & publish CLI."),
