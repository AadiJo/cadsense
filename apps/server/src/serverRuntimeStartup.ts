...
import { CadReviewReactor } from "./orchestration/Services/CadReviewService.ts";
...
export const makeServerRuntimeStartup = Effect.gen(function* () {
  ...
  const cadReviewReactor = yield* CadReviewReactor;
  ...
  const startup = Effect.gen(function* () {
    ...
    yield* runStartupPhase(
      ...
      Effect.gen(function* () {
        ...
        yield* cadReviewReactor.start().pipe(Scope.provide(reactorScope));
      ...
      if (serverConfig.startupPresentation === "headless") {
        ...
        if (serverConfig.mode !== "desktop") {
          yield* Effect.logInfo(
            "Authentication required. Open CadSense using the pairing URL.",
