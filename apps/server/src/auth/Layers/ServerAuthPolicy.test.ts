  ...
  ServerAuthPolicyLive.pipe(
    Layer.provide(
      Layer.effect(
        ...
        Layer.provide(
          ServerConfig.layerTest(process.cwd(), { prefix: "cadsense-auth-policy-test-" }),
        ),
      ),
    ),
  );
...
it.layer(NodeServices.layer)("ServerAuthPolicyLive", (it) => {
  it.effect("uses desktop-managed-local policy for desktop mode", () =>
    Effect.gen(function* () {
      ...
      expect(descriptor.sessionCookieName).toBe("cadsense_session_3773");
    }).pipe(
      Effect.provide(
  ...
  it.effect("uses loopback-browser policy for loopback web hosts", () =>
    Effect.gen(function* () {
      ...
      expect(descriptor.sessionCookieName).toBe("cadsense_session");
    }).pipe(
      Effect.provide(
        makeServerAuthPolicyLayer({
          mode: "web",
