...
import { CodexSettings, ProviderInstanceId, TextGenerationError } from "@cadsense/contracts";
...
const CodexTextGenerationTestLayer = ServerConfig.layerTest(process.cwd(), {
  prefix: "cadsensex-text-generation-test-",
  ...
  return Effect.gen(function* () {
    ...
    yield* fs.writeFileString(
      ...
      [
        ...
        "  cat > \"$output_path\" <<'__CADSENSE_FAKE_CODEX_OUTPUT__'",
        input.output,
        "__CADSENSE_FAKE_CODEX_OUTPUT__",
        "fi",
...
function withFakeCodexEnv<A, E, R>(
  ...
  return Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
    const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "cadsensex-text-" });
