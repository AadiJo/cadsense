...
const makeDesktopBootstrap = (
  ...
  noBrowser: true,
  port: 4888,
  cadsenseHome: "/tmp/cadsense-bootstrap-home",
  host: "127.0.0.1",
...
it.layer(NodeServices.layer)("cli config resolution", (it) => {
  ...
  it.effect("falls back to effect/config values when flags are omitted", () =>
    Effect.gen(function* () {
      ...
      const baseDir = join(NodeOS.tmpdir(), "cadsense-cli-config-env-base");
          ...
          Layer.mergeAll(
            ConfigProvider.layer(
              ConfigProvider.fromEnv({
                env: {
                  CADSENSE_LOG_LEVEL: "Warn",
                  CADSENSE_MODE: "desktop",
                  CADSENSE_PORT: "4001",
                  CADSENSE_HOST: "0.0.0.0",
                  CADSENSE_HOME: baseDir,
                  VITE_DEV_SERVER_URL: "http://127.0.0.1:5173",
                  CADSENSE_NO_BROWSER: "true",
                  CADSENSE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "false",
                  CADSENSE_LOG_WS_EVENTS: "true",
                },
              }),
            ),
            NetService.layer,
  ...
  it.effect("uses CLI flags when provided", () =>
    Effect.gen(function* () {
      const { join } = yield* Path.Path;
      const baseDir = join(NodeOS.tmpdir(), "cadsense-cli-config-flags-base");
            ...
            ConfigProvider.layer(
              ConfigProvider.fromEnv({
                env: {
                  ...
                  CADSENSE_PORT: "4001",
                  CADSENSE_HOST: "0.0.0.0",
                  CADSENSE_HOME: join(NodeOS.tmpdir(), "ignored-base"),
                  VITE_DEV_SERVER_URL: "http://127.0.0.1:5173",
                  CADSENSE_NO_BROWSER: "false",
                  CADSENSE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "false",
                  CADSENSE_LOG_WS_EVENTS: "false",
                },
              }),
            ),
            NetService.layer,
  ...
  it.effect("preserves explicit false CLI boolean flags over env and bootstrap values", () =>
    Effect.gen(function* () {
      const { join } = yield* Path.Path;
      const baseDir = join(NodeOS.tmpdir(), "cadsense-cli-config-false-flags");
        ...
        Effect.provide(
          Layer.mergeAll(
            ConfigProvider.layer(
              ConfigProvider.fromEnv({
                env: {
                  CADSENSE_BOOTSTRAP_FD: String(fd),
                  CADSENSE_NO_BROWSER: "true",
                  CADSENSE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "true",
                  CADSENSE_LOG_WS_EVENTS: "true",
                },
              }),
            ),
  ...
  it.effect("creates derived runtime directories during config resolution", () =>
    Effect.gen(function* () {
      ...
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "cadsense-cli-config-dirs-" });
  ...
  it.effect("applies flag then env precedence over bootstrap envelope values", () =>
    Effect.gen(function* () {
      const { join } = yield* Path.Path;
      const baseDir = join(NodeOS.tmpdir(), "cadsense-cli-config-env-wins");
      const fd = yield* openBootstrapFd(
        makeDesktopBootstrap({
          port: 4888,
          host: "127.0.0.2",
  ...
  it.effect("falls back to persisted observability settings when env vars are absent", () =>
    Effect.gen(function* () {
      ...
      const baseDir = yield* fs.makeTempDirectoryScoped({
        prefix: "cadsense-cli-config-settings-",
      });
      const derivedPaths = yield* deriveServerPaths(baseDir, undefined);
  ...
  it.effect("forces noBrowser and disables auto-bootstrap for headless startup presentation", () =>
    Effect.gen(function* () {
      const { join } = yield* Path.Path;
      const baseDir = join(NodeOS.tmpdir(), "cadsense-cli-config-headless-base");
      const derivedPaths = yield* deriveServerPaths(baseDir, undefined);
