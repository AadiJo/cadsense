...
export const baseDirFlag = Flag.string("base-dir").pipe(
  Flag.withDescription("Base directory path (equivalent to CADSENSE_HOME)."),
  Flag.optional,
);
...
export const logWebSocketEventsFlag = Flag.boolean("log-websocket-events").pipe(
  Flag.withDescription(
    "Emit server-side logs for outbound WebSocket push traffic (equivalent to CADSENSE_LOG_WS_EVENTS).",
  ),
...
const EnvServerConfig = Config.all({
  ...
  traceBatchWindowMs: Config.int("CADSENSE_TRACE_BATCH_WINDOW_MS").pipe(Config.withDefault(200)),
  ...
  mode: Config.schema(RuntimeMode, "CADSENSE_MODE").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  port: Config.port("CADSENSE_PORT").pipe(Config.option, Config.map(Option.getOrUndefined)),
  host: Config.string("CADSENSE_HOST").pipe(Config.option, Config.map(Option.getOrUndefined)),
  cadsenseHome: Config.string("CADSENSE_HOME").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  devUrl: Config.url("VITE_DEV_SERVER_URL").pipe(Config.option, Config.map(Option.getOrUndefined)),
  noBrowser: Config.boolean("CADSENSE_NO_BROWSER").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  ...
  tailscaleServeEnabled: Config.boolean("CADSENSE_TAILSCALE_SERVE").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  tailscaleServePort: Config.port("CADSENSE_TAILSCALE_SERVE_PORT").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
});
    ...
    const baseDir = yield* resolveBaseDir(
      Option.getOrUndefined(
        resolveOptionPrecedence(
          ...
          Option.fromUndefinedOr(bootstrap?.cadsenseHome),
        ),
      ),
    );
