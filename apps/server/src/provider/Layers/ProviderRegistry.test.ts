...
import { deepMerge } from "@cadsense/shared/Struct";
...
process.env.CADSENSE_CURSOR_ENABLED = "1";
  ...
  (it) => {
    ...
    describe("ProviderRegistryLive", () => {
            ...
            ProviderRegistryLive.pipe(
              ...
              Layer.provideMerge(
                ServerConfig.layerTest(process.cwd(), {
                  prefix: "cadsense-provider-registry-merged-persist-",
                }),
              ),
        ...
        Effect.gen(function* () {
          ...
          const runtimeServices = yield* Layer.build(
            ProviderRegistryLive.pipe(
              ...
              Layer.provideMerge(
                ServerConfig.layerTest(process.cwd(), {
                  prefix: "cadsense-provider-registry-refresh-failure-",
                }),
              ),
              Layer.provideMerge(NodeServices.layer),
            ),
        ...
        Effect.gen(function* () {
          ...
          const runtimeServices = yield* Layer.build(
            ProviderRegistryLive.pipe(
              ...
              Layer.provideMerge(
                ServerConfig.layerTest(process.cwd(), {
                  prefix: "cadsense-provider-registry-sync-failure-",
                }),
              ),
              Layer.provideMerge(NodeServices.layer),
            ),
      ...
      it.effect("propagates real Codex probe failures to the aggregator at boot", () =>
        Effect.gen(function* () {
          const missingBinary = `cadsense_codex_missing_`;
          const serverSettings = yield* makeMutableServerSettingsService(
            decodeServerSettings(
              deepMerge(encodedDefaultServerSettings, {
                ...
                providerInstances: {
                  // Matches the shape the user had in `.cadsense/dev/settings.json`
          ...
          const providerRegistryLayer = ProviderRegistryLive.pipe(
            ...
            Layer.provideMerge(
              ServerConfig.layerTest(process.cwd(), {
                prefix: "cadsense-provider-registry-",
              }),
            ),
      ...
      it.effect("re-probes when settings change the codex binaryPath", () =>
        Effect.gen(function* () {
          const firstMissing = `cadsense_codex_first_`;
          const secondMissing = `cadsense_codex_second_`;
          const serverSettings = yield* makeMutableServerSettingsService(
            decodeServerSettings(
              deepMerge(encodedDefaultServerSettings, {
                providers: {
          ...
          Effect.gen(function* () {
            ...
            yield* Effect.gen(function* () {
              ...
              assert.strictEqual(
                ...
                "Cursor is disabled in CadSense settings.",
              );
      ...
      it.effect("skips codex probes entirely when the provider is disabled", () =>
        Effect.gen(function* () {
          ...
          assert.strictEqual(status.installed, false);
          assert.strictEqual(status.message, "Codex is disabled in CadSense settings.");
        }),
      );
    });
    ...
    describe("checkClaudeProviderStatus", () => {
      ...
      it.effect("runs Claude status probes with the configured Claude HOME", () => {
        ...
        return Effect.gen(function* () {
          ...
          const claudeHome = path.resolve("/tmp/cadsense-claude-home");
          const status = yield* checkClaudeProviderStatus(
            {
              ...defaultClaudeSettings,
              homePath: claudeHome,
