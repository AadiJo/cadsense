...
describe("openCodexThread", () => {
  it("passes codexThreadStartConfig as Codex thread config", async () => {
    ...
    await Effect.runPromise(
      openCodexThread({
        ...
        codexThreadStartConfig: { mcp_servers: { "cadsense-cad-view": { command: "bun" } } },
      }),
    );

    assert.equal(calls.length, 1);
    assert.equal(calls[0]!.method, "thread/start");
    assert.deepStrictEqual((calls[0]!.payload as { config?: unknown }).config, {
      mcp_servers: { "cadsense-cad-view": { command: "bun" } },
    });
  });
