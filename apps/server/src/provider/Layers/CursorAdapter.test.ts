  ...
  const script = isWindows
    ? [
        ...
        ...(options?.initialDelaySeconds
          ? [`powershell -NoProfile -Command "Start-Sleep -Seconds ${options.initialDelaySeconds}"`]
          : []),
        `${bunExe} "${mockAgentPath}" %*`,
      ].join("\r\n")
    : `#!/bin/sh
...
async function makeProbeWrapper(
  ...
  const script = isWindows
    ? [
        "@echo off",
        `echo %*>> "${argvLogPath}"`,
        `set CADSENSE_ACP_REQUEST_LOG_PATH=${requestLogPath}`,
        ...Object.entries(extraEnv ?? {}).map(([key, value]) => `set ${key}=${value}`),
        `${bunExe} "${mockAgentPath}" %*`,
      ].join("\r\n")
    : `#!/bin/sh
...
export CADSENSE_ACP_REQUEST_LOG_PATH=${JSON.stringify(requestLogPath)}
${Object.entries(extraEnv ?? {})
  .map(([key, value]) => `export ${key}=${JSON.stringify(value)}`)
  .join("\n")}
exec ${JSON.stringify(bunExe)} ${JSON.stringify(mockAgentPath)} "$@"
  ...
  Layer.effect(
    ...
    Layer.provideMerge(
      ServerConfig.layerTest(process.cwd(), {
        prefix: "cadsense-cursor-adapter-test-",
      }),
    ),
...
cursorAdapterTestLayer("CursorAdapterLive", (it) => {
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        const previousEmitToolCalls = process.env.CADSENSE_ACP_EMIT_TOOL_CALLS;
        process.env.CADSENSE_ACP_EMIT_TOOL_CALLS = "1";
        ...
        const wrapperPath = yield* Effect.promise(() =>
          makeMockAgentWrapper({ CADSENSE_ACP_EMIT_TOOL_CALLS: "1" }),
        );
        yield* serverSettings.updateSettings({
          providers: { cursor: { binaryPath: wrapperPath } },
        });
          ...
          Effect.ensuring(
            Effect.sync(() => {
              if (previousEmitToolCalls === undefined) {
                delete process.env.CADSENSE_ACP_EMIT_TOOL_CALLS;
              } else {
                process.env.CADSENSE_ACP_EMIT_TOOL_CALLS = previousEmitToolCalls;
              }
            }),
          ),
        );
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* Effect.promise(() => writeFile(requestLogPath, "", "utf8"));
        const wrapperPath = yield* Effect.promise(() =>
          makeProbeWrapper(requestLogPath, argvLogPath, { CADSENSE_ACP_EMIT_TOOL_CALLS: "1" }),
        );
        yield* serverSettings.updateSettings({
          providers: { cursor: { binaryPath: wrapperPath } },
        });
  ...
  it.effect("segments assistant messages around ACP tool activity in full-access mode", () =>
    Effect.gen(function* () {
      ...
      const wrapperPath = yield* Effect.promise(() =>
        makeMockAgentWrapper({ CADSENSE_ACP_EMIT_INTERLEAVED_ASSISTANT_TOOL_CALLS: "1" }),
      );
      yield* serverSettings.updateSettings({
        providers: { cursor: { binaryPath: wrapperPath } },
      });
  ...
  it.effect("stopping a session settles pending user-input waits", () =>
    Effect.gen(function* () {
      ...
      const wrapperPath = yield* Effect.promise(() =>
        makeMockAgentWrapper({ CADSENSE_ACP_EMIT_ASK_QUESTION: "1" }),
      );
      yield* serverSettings.updateSettings({ providers: { cursor: { binaryPath: wrapperPath } } });
  ...
  it.effect(
    ...
    () => {
      ...
      const customAdapterLayer = Layer.effect(
        ...
        Layer.provideMerge(
          ServerConfig.layerTest(process.cwd(), {
            prefix: "cadsense-cursor-adapter-custom-instance-",
          }),
        ),
        Layer.provideMerge(NodeServices.layer),
      );
