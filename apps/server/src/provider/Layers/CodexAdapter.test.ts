...
lifecycleLayer("CodexAdapterLive lifecycle", (it) => {
  ...
  it.effect("maps reasoning text deltas to canonical content deltas", () =>
    Effect.gen(function* () {
      const { adapter, runtime } = yield* startLifecycleRuntime();
      ...
      yield* runtime.emit({
        id: asEventId("evt-reasoning-delta"),
        kind: "notification",
        provider: ProviderDriverKind.make("codex"),
        createdAt: "2026-01-01T00:00:00.000Z",
      ...
      yield* runtime.emit({
        ...
        turnId: asTurnId("turn-1"),
        itemId: asItemId("reasoning_1"),
        textDelta: "summary token",
      } satisfies ProviderEvent);

      const events = Array.from(yield* Fiber.join(eventsFiber));
      assert.deepStrictEqual(
        events.map((event) => (event.type === "content.delta" ? event.payload.streamKind : "")),
        ["reasoning_text", "reasoning_summary_text"],
      );
      assert.deepStrictEqual(
        events.map((event) => (event.type === "content.delta" ? event.payload.delta : "")),
        ["thinking token", "summary token"],
      );
    }),
  );

  it.effect(
    "maps agent message deltas whose wire payload nests text under `{ text }` to assistant content deltas",
    () =>
      Effect.gen(function* () {
        const { adapter, runtime } = yield* startLifecycleRuntime();
        ...
        yield* runtime.emit({
          id: asEventId("evt-agent-msg-structured"),
          kind: "notification",
          provider: ProviderDriverKind.make("codex"),
          createdAt: "2026-01-01T00:00:00.000Z",
          ...
          threadId: asThreadId("thread-1"),
          turnId: asTurnId("turn-1"),
          itemId: asItemId("assistant_1"),
          payload: {
            threadId: "thread-1",
            turnId: "turn-1",
            itemId: "assistant_1",
            delta: { text: "MCP servers: cadsense-cad-view.\n" },
          },
        } satisfies ProviderEvent);
        ...
        assert.equal(event.value.type, "content.delta");
        if (event.value.type !== "content.delta") {
          return;
        }
        assert.equal(event.value.payload.streamKind, "assistant_text");
        assert.equal(event.value.payload.delta, "MCP servers: cadsense-cad-view.\n");
      }),
  );
  ...
  it.effect("maps MCP tool call progress when `message` is a structured blob carrying text", () =>
    Effect.gen(function* () {
      ...
      yield* runtime.emit({
        id: asEventId("evt-mcp-progress-structured"),
        kind: "notification",
        provider: ProviderDriverKind.make("codex"),
        createdAt: "2026-01-01T00:00:00.000Z",
        ...
        payload: {
          ...
          turnId: "turn-1",
          itemId: "mcp_call_1",
          message: { text: "Listing MCP tools..." },
        },
      } satisfies ProviderEvent);
      ...
      assert.equal(event.value.type, "tool.progress");
      if (event.value.type !== "tool.progress") {
        return;
      }
      assert.equal(event.value.payload.summary, "Listing MCP tools...");
    }),
  );
  ...
  it.effect("maps turn/aborted lifecycle events to canonical turn.aborted runtime events", () =>
    Effect.gen(function* () {
      const { adapter, runtime } = yield* startLifecycleRuntime();
      const firstEventFiber = yield* Stream.runHead(adapter.streamEvents).pipe(Effect.forkChild);
      ...
      assert.equal(firstEvent.value.threadId, "thread-1");
      assert.equal(firstEvent.value.turnId, "turn-1");
      assert.equal(firstEvent.value.payload.reason, "interrupted");
    }),
  );
  ...
  it.effect("maps thread/realtime/transcript/delta to assistant content.delta", () =>
    Effect.gen(function* () {
      const { adapter, runtime } = yield* startLifecycleRuntime();
      ...
      yield* runtime.emit({
        id: asEventId("evt-rt-transcript-delta"),
        kind: "notification",
        provider: ProviderDriverKind.make("codex"),
        threadId: asThreadId("thread-1"),
        createdAt: "2026-01-01T00:00:00.000Z",
      ...
      if (first?.type !== "content.delta") {
        return;
      }
      assert.equal(first.payload.streamKind, "assistant_text");
      assert.equal(first.payload.delta, "Hello");
    }),
  );

  it.effect("maps thread/realtime/transcript/done to assistant content.delta", () =>
    Effect.gen(function* () {
      const { adapter, runtime } = yield* startLifecycleRuntime();
      const eventsFiber = yield* Stream.runCollect(Stream.take(adapter.streamEvents, 1)).pipe(
        Effect.forkChild,
      ...
      yield* runtime.emit({
        id: asEventId("evt-rt-transcript-done"),
        kind: "notification",
        provider: ProviderDriverKind.make("codex"),
        threadId: asThreadId("thread-1"),
        createdAt: "2026-01-01T00:00:00.000Z",
      ...
      assert.equal(first.payload.streamKind, "assistant_text");
      assert.equal(first.payload.delta, "Final answer.");
    }),
  );
...
it.effect("flushes managed native logs when the adapter layer shuts down", () =>
  Effect.gen(function* () {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "cadsensex-adapter-native-log-"));
