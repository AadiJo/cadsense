...
export interface CodexSessionRuntimeOptions {
  ...
  /**
   * Merged into Codex `thread/start` and `thread/resume` as `config` (e.g. `mcp_servers` for the
   * bundled `cadsense-cad-view` Onshape side-panel MCP).
   */
  readonly codexThreadStartConfig?: Record<string, unknown>;
}
...
function buildThreadStartParams(input: {
  ...
  return {
    ...
    ...(input.codexThreadStartConfig ? { config: input.codexThreadStartConfig } : {}),
  };
}
...
export const openCodexThread = (input: {
  ...
  const startParams = buildThreadStartParams({
    ...
    ...(input.codexThreadStartConfig !== undefined
      ? { codexThreadStartConfig: input.codexThreadStartConfig }
      : {}),
  });
  ...
  Effect.gen(function* () {
      ...
      Effect.gen(function* () {
        ...
        yield* emitEvent({
          ...
          ...(notification.method === "item/agentMessage/delta"
            ? { textDelta: normalizeCodexStreamDelta(notification.params.delta) }
            : notification.method === "thread/realtime/transcript/delta"
              ? { textDelta: normalizeCodexStreamDelta(notification.params.delta) }
              : notification.method === "thread/realtime/transcript/done"
                ? { textDelta: normalizeCodexTranscriptSnippet(notification.params.text) }
                : {}),
    ...
    const start = Effect.fn("CodexSessionRuntime.start")(function* () {
      ...
      const opened = yield* openCodexThread({
        ...
        ...(options.codexThreadStartConfig !== undefined
          ? { codexThreadStartConfig: options.codexThreadStartConfig }
          : {}),
      });
