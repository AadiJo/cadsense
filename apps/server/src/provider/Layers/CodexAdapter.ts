...
export interface CodexAdapterLiveOptions {
  ...
  /** Resolved directory where CAD MCP screenshots are written (Codex home / shadow). */
...
function readUnknownCodexWireRecord(
  payload: ProviderEvent["payload"],
...
function resolveCodexPayloadDelta(event: ProviderEvent): string | undefined {
  ...
  return (
    normalizeCodexStreamDelta(event.textDelta) ?? normalizeCodexStreamDelta(raw?.delta) ?? undefined
  );
}

function resolveCodexRealtimeTranscriptSnippet(event: ProviderEvent): string | undefined {
  const raw = readUnknownCodexWireRecord(event.payload);
  const combined =
    normalizeCodexStreamDelta(event.textDelta) ??
    normalizeCodexStreamDelta(raw?.delta) ??
    normalizeCodexTranscriptSnippet(raw?.text);
  return combined;
}
...
function mapToRuntimeEvents(
  ...
  if (event.method === "item/plan/delta") {
    ...
    const delta = resolveCodexPayloadDelta(event) ?? normalizeCodexStreamDelta(payload?.delta);
  ...
  if (event.method === "item/mcpToolCall/progress") {
    ...
    const loose = readUnknownCodexWireRecord(event.payload);
    const summary = trimText(
      normalizeCodexStreamDelta(loose?.message) ?? normalizeCodexStreamDelta(payload?.message),
    );
    if (!summary) {
      return [];
    }
  ...
  if (event.method === "thread/realtime/transcript/delta") {
    ...
    const loose = readUnknownCodexWireRecord(event.payload);
    const delta = resolveCodexRealtimeTranscriptSnippet(event);
    if (!delta || delta.length === 0) {
      return [];
    }
    const role = (
      trimText(payload?.role) ?? trimText(typeof loose?.role === "string" ? loose.role : undefined)
    )?.toLowerCase();
    if (role === "user") {
      return [];
    }
  ...
  if (event.method === "thread/realtime/transcript/done") {
    ...
    const loose = readUnknownCodexWireRecord(event.payload);
    const textSnippet = trimText(resolveCodexRealtimeTranscriptSnippet(event));
    if (!textSnippet) {
      return [];
    }
    const role = (
