...
import { GitCommandError } from "@cadsense/contracts";
import { ServerConfig } from "../config.ts";
import * as GitVcsDriver from "./GitVcsDriver.ts";

const ServerConfigLayer = ServerConfig.layerTest(process.cwd(), {
  prefix: "cadsense-git-vcs-driver-test-",
});
  ...
  describe("repository status", () => {
    ...
    it.effect("disables SSH askpass for background upstream status fetches", () =>
      Effect.gen(function* () {
        ...
        const previousAskpassLog = process.env.CADSENSE_TEST_SSH_ASKPASS_LOG;

        yield* fileSystem.writeFileString(
          sshWrapperPath,
          [
            "#!/bin/sh",
            'printf "%s\\n" "${SSH_ASKPASS_REQUIRE:-}" > "$CADSENSE_TEST_SSH_ASKPASS_LOG"',
            "exit 1",
            "",
          ].join("\n"),
        );
        ...
        yield* Effect.gen(function* () {
          ...
          process.env.CADSENSE_TEST_SSH_ASKPASS_LOG = sshLogPath;
          ...
          Effect.ensuring(
            Effect.sync(() => {
              ...
              if (previousAskpassLog === undefined) {
                delete process.env.CADSENSE_TEST_SSH_ASKPASS_LOG;
              } else {
                process.env.CADSENSE_TEST_SSH_ASKPASS_LOG = previousAskpassLog;
              }
            }),
          ),
        );
