...
import { GitCommandError, type VcsRef } from "@cadsense/contracts";
import { dedupeRemoteBranchesWithLocalMatches } from "@cadsense/shared/git";
import { compactTraceAttributes } from "@cadsense/shared/observability";
...
function pathAfterFields(line: string, fieldCount: number): string | null {
  ...
  const filePath = line.slice(offset);
...
function parsePorcelainPath(line: string): string | null {
  ...
  if (line.startsWith("1 ")) return pathAfterFields(line, 8);
  if (line.startsWith("2 ")) return pathAfterFields(line, 9);
  if (line.startsWith("u ")) return pathAfterFields(line, 10);
  return null;
}
...
> {
  ...
  const traceFilePath = yield* fs.makeTempFileScoped({
    prefix: `cadsense-git-trace2-${process.pid}-`,
    suffix: ".json",
  });
