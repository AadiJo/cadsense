...
import { VcsDriverKind, type VcsDriverKind as VcsDriverKindType } from "@cadsense/contracts";
...
export const make = Effect.fn("makeVcsProjectConfig")(function* () {
  ...
  const findConfigPath = Effect.fn("VcsProjectConfig.findConfigPath")(function* (cwd: string) {
    let current = cwd;
    while (true) {
      const candidate = path.join(current, ".cadsense", "vcs.json");
