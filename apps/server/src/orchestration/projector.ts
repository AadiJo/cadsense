  ...
  switch (event.type) {
            ...
            project.id === payload.projectId
              ? {
                  ...
                  ...(payload.workspaceRoot !== undefined
                    ? { workspaceRoot: payload.workspaceRoot }
                    : {}),
                  ...(payload.externalContext !== undefined
    ...
    case "thread.review-requested":
      return Effect.gen(function* () {
        ...
        } satisfies CadReviewReport;
        return {
          ...nextBase,
          threads: updateThread(nextBase.threads, payload.threadId, {
            reviews: [
              ...(thread.reviews ?? []).filter((entry) => entry.id !== payload.reviewRunId),
              requestedReview,
            ].toSorted(
              (left, right) =>
                left.createdAt.localeCompare(right.createdAt) || left.id.localeCompare(right.id),
    ...
    case "thread.review-upserted":
      return Effect.gen(function* () {
        ...
        const reviews = [
          ...(thread.reviews ?? []).filter((entry) => entry.id !== payload.review.id),
          payload.review,
        ]
          .toSorted(
            (left, right) =>
