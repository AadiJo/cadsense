...
import { ProjectionThreadReview } from "../../persistence/Services/ProjectionThreadReviews.ts";
...
const ProjectionProjectDbRowSchema = ProjectionProject.mapFields(
  Struct.assign({
    externalContext: Schema.NullOr(Schema.fromJsonString(ExternalProjectContext)),
...
const ProjectionThreadReviewDbRowSchema = ProjectionThreadReview.mapFields(
  Struct.assign({
...
const ProjectionThreadDbRowSchema = ProjectionThread.mapFields(
  Struct.assign({
    externalContext: Schema.NullOr(Schema.fromJsonString(ExternalThreadContext)),
...
const ActiveReviewThreadRowSchema = Schema.Struct({
  threadId: ThreadId,
...
const REQUIRED_SNAPSHOT_PROJECTORS = [
  ...
  ORCHESTRATION_PROJECTOR_NAMES.threadReviews,
...
function mapReviewRow(
  row: Schema.Schema.Type<typeof ProjectionThreadReviewDbRowSchema>,
): CadReviewReport {
  return row.review;
}
...
const makeProjectionSnapshotQuery = Effect.gen(function* () {
  ...
  const listThreadReviewRows = SqlSchema.findAll({
    Request: Schema.Void,
    Result: ProjectionThreadReviewDbRowSchema,
    execute: () =>
      sql`
        SELECT
        ...
        ORDER BY thread_id ASC, created_at ASC, review_id ASC
      `,
  });

  const listActiveReviewThreadRows = SqlSchema.findAll({
    Request: Schema.Void,
    Result: ActiveReviewThreadRowSchema,
    execute: () =>
      sql`
        SELECT
          thread_id AS "threadId",
        ...
        FROM projection_thread_reviews
        WHERE json_extract(review, '$.status') NOT IN ('completed', 'partial', 'failed')
        GROUP BY thread_id
      `,
  });
  ...
  const listThreadReviewRowsByThread = SqlSchema.findAll({
    Request: ThreadIdLookupInput,
    Result: ProjectionThreadReviewDbRowSchema,
    execute: ({ threadId }) =>
      sql`
  ...
  const listActiveReviewThreadRowsByThread = SqlSchema.findAll({
    Request: ThreadIdLookupInput,
    Result: ActiveReviewThreadRowSchema,
    execute: ({ threadId }) =>
      sql`
        ...
        FROM projection_thread_reviews
        WHERE thread_id = ${threadId}
          AND json_extract(review, '$.status') NOT IN ('completed', 'partial', 'failed')
        GROUP BY thread_id
      `,
  });
    ...
    sql
      .withTransaction(
        Effect.all([
          ...
          listThreadReviewRows(undefined).pipe(
            Effect.mapError(
              toPersistenceSqlOrDecodeError(
                "ProjectionSnapshotQuery.getSnapshot:listThreadReviews:query",
                "ProjectionSnapshotQuery.getSnapshot:listThreadReviews:decodeRows",
              ),
            ),
          ),
        ...
        Effect.flatMap(
          ([
            ...
            Effect.gen(function* () {
              ...
              const reviewsByThread = new Map<string, Array<CadReviewReport>>();
              ...
              for (const row of reviewRows) {
                updatedAt = maxIso(updatedAt, row.updatedAt);
                const threadReviews = reviewsByThread.get(row.threadId) ?? [];
                threadReviews.push(mapReviewRow(row));
                reviewsByThread.set(row.threadId, threadReviews);
              }
              ...
              const threads: ReadonlyArray<OrchestrationThread> = threadRows.map((row) => ({
                ...
                reviews: reviewsByThread.get(row.threadId) ?? [],
      ...
      .withTransaction(
        Effect.all([
          ...
          listThreadReviewRows(undefined).pipe(
            Effect.mapError(
              toPersistenceSqlOrDecodeError(
                "ProjectionSnapshotQuery.getCommandReadModel:listThreadReviews:query",
                "ProjectionSnapshotQuery.getCommandReadModel:listThreadReviews:decodeRows",
              ),
            ),
          ),
          listThreadSessionRows(undefined).pipe(
    ...
    sql
      .withTransaction(
        Effect.all([
          ...
          listActiveReviewThreadRows(undefined).pipe(
            Effect.mapError(
              toPersistenceSqlOrDecodeError(
                "ProjectionSnapshotQuery.getShellSnapshot:listActiveReviewThreads:query",
                "ProjectionSnapshotQuery.getShellSnapshot:listActiveReviewThreads:decodeRows",
              ),
            ),
          ),
        ]),
      ...
      .pipe(
        Effect.flatMap(
          ([projectRows, threadRows, sessionRows, latestTurnRows, stateRows, activeReviewRows]) =>
            Effect.gen(function* () {
              const activeReviewThreadIds = new Set(activeReviewRows.map((row) => row.threadId));
              let updatedAt: string | null = null;
              for (const row of projectRows) {
                updatedAt = maxIso(updatedAt, row.updatedAt);
              }
              ...
              for (const row of activeReviewRows) {
                updatedAt = maxIso(updatedAt, row.updatedAt);
              }
                ...
                threads: threadRows
                  ...
                  .map(
                    (row): OrchestrationThreadShell => ({
                      ...
                      hasActiveReview: activeReviewThreadIds.has(row.threadId),
                    }),
                  ),
                updatedAt: updatedAt ?? "1970-01-01T00:00:00.000Z",
              };
      ...
      .withTransaction(
        Effect.all([
          ...
          listActiveReviewThreadRows(undefined).pipe(
            Effect.mapError(
              toPersistenceSqlOrDecodeError(
                "ProjectionSnapshotQuery.getArchivedShellSnapshot:listActiveReviewThreads:query",
                "ProjectionSnapshotQuery.getArchivedShellSnapshot:listActiveReviewThreads:decodeRows",
              ),
            ),
          ),
        ]),
            ...
            : repositoryIdentityResolver.resolve(option.value.workspaceRoot).pipe(
                Effect.map((repositoryIdentity) =>
                  Option.some({
                    ...
                    repositoryIdentity,
                    externalContext: option.value.externalContext,
  ...
  const getThreadShellById: ProjectionSnapshotQueryShape["getThreadShellById"] = (threadId) =>
    Effect.gen(function* () {
      const [threadRow, latestTurnRow, sessionRow, activeReviewRows] = yield* Effect.all([
        ...
        listActiveReviewThreadRowsByThread({ threadId }).pipe(
          Effect.mapError(
            toPersistenceSqlOrDecodeError(
              "ProjectionSnapshotQuery.getThreadShellById:listActiveReviewThreads:query",
              "ProjectionSnapshotQuery.getThreadShellById:listActiveReviewThreads:decodeRows",
            ),
          ),
        ),
      ]);
      ...
      return Option.some({
        ...
        hasActiveReview: activeReviewRows.length > 0,
  ...
  const getThreadDetailById: ProjectionSnapshotQueryShape["getThreadDetailById"] = (threadId) =>
    Effect.gen(function* () {
      const [
        ...
        listThreadReviewRowsByThread({ threadId }).pipe(
          Effect.mapError(
            toPersistenceSqlOrDecodeError(
              "ProjectionSnapshotQuery.getThreadDetailById:listReviews:query",
              "ProjectionSnapshotQuery.getThreadDetailById:listReviews:decodeRows",
            ),
          ),
        ),
