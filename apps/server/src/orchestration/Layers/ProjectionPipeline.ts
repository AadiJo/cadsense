...
import { ProjectionThreadReviewRepository } from "../../persistence/Services/ProjectionThreadReviews.ts";
...
import { ProjectionThreadReviewRepositoryLive } from "../../persistence/Layers/ProjectionThreadReviews.ts";
...
export const ORCHESTRATION_PROJECTOR_NAMES = {
  ...
  threadReviews: "projection.thread-reviews",
...
const makeOrchestrationProjectionPipeline = Effect.fn("makeOrchestrationProjectionPipeline")(
  function* () {
    ...
    const projectionThreadReviewRepository = yield* ProjectionThreadReviewRepository;
      ...
      switch (event.type) {
        case "project.created":
          yield* projectionProjectRepository.upsert({
            ...
            externalContext: event.payload.externalContext ?? null,
        ...
        case "project.meta-updated": {
          ...
          yield* projectionProjectRepository.upsert({
            ...
            ...(event.payload.workspaceRoot !== undefined
              ? { workspaceRoot: event.payload.workspaceRoot }
              : {}),
            ...(event.payload.externalContext !== undefined
              ? { externalContext: event.payload.externalContext }
              : {}),
      ...
      switch (event.type) {
        case "thread.created":
          yield* projectionThreadRepository.upsert({
            ...
            projectId: event.payload.projectId,
            purpose: event.payload.purpose ?? "general",
            parentThreadId: event.payload.parentThreadId ?? null,
            reviewRunId: event.payload.reviewRunId ?? null,
            title: event.payload.title,
            externalContext: event.payload.externalContext ?? null,
          ...
          if (event.payload.externalContext?.provider === "onshape") {
            // @effect-diagnostics-next-line preferSchemaOverJson:off
            const contextJson = JSON.stringify(event.payload.externalContext.onshape);
            yield* sql`
            ...
            `.pipe(
              Effect.catchTag("SqlError", (sqlError) =>
                Effect.fail(
                  toPersistenceSqlError("ProjectionPipeline.upsertOnshapeThreadContext:query")(
                    sqlError,
                  ),
                ),
              ),
    ...
    const applyThreadReviewsProjection: ProjectorDefinition["apply"] = Effect.fn(
      "applyThreadReviewsProjection",
    )(function* (event, _attachmentSideEffects) {
      switch (event.type) {
        case "thread.review-requested":
          yield* projectionThreadReviewRepository.upsert({
            reviewId: event.payload.reviewRunId,
            threadId: event.payload.threadId,
            review: {
        ...
        case "thread.review-upserted":
          yield* projectionThreadReviewRepository.upsert({
            ...
            createdAt: event.payload.review.createdAt,
    ...
    const projectors: ReadonlyArray<ProjectorDefinition> = [
      ...
      {
        name: ORCHESTRATION_PROJECTOR_NAMES.threadReviews,
        apply: applyThreadReviewsProjection,
      },
      {
...
export const OrchestrationProjectionPipelineLive = Layer.effect(
  ...
  Layer.provideMerge(ProjectionThreadReviewRepositoryLive),
