...
export function buildReviewerPrompt(input: {
  ...
  return [
    ...
    "Do not call request_user_input, ask_question, or any other hidden-user-question tool. If you need user context, state the assumption and put the unanswered question in missingEvidence or specificCheck.",
    "A topConcern must be a judgment about geometry you actually inspected, not a general design reminder. Do not write reminders such as 'make sure the belt ratio is correct' as concerns unless you can explain whether the current ratio, spacing, or layout is good or bad for the visible use case.",
    "For visible power transmission, judge the actual design instead of stopping at packaging. If pulley, sprocket, or gear tooth counts are visible, calculate the combined ratio and explain whether the resulting speed/torque is good or bad for the assumed mechanism role.",
    ...
    "Separate checks from findings: if the CAD only needs a measurement, calculation, or user context before you can judge it, put that in missingEvidence or specificCheck instead of presenting it as a warning.",
    ...
    "If search returns a promising image artifact, use fetch_mechbase_artifact only when seeing the image would materially affect your recommendation. Use or cite the image only after checking that it visibly matches the mechanism or precedent you are discussing.",
    ...
    "Use plain student-facing language in every title and description. Avoid compressed mechanical shorthand; if you use a technical term, immediately tie it to the visible part and the practical consequence.",
  ].join("\n");
}
...
export function buildMechanismPlanningPrompt(input: {
  ...
  return [
    ...
    "Do not call request_user_input, ask_question, or any other hidden-user-question tool from this review workflow. If the mechanism role or success criteria are unclear, make the best visible-geometry assumption and put the question in missingContext instead of blocking the review.",
    "Do not treat the phrase 'in depth' as a request for broader scope by itself. 'In depth' means a deeper review of the visible/current CAD context; only broaden to full-assembly review when the user asks for holistic, all-around, full robot, all angles, or similar breadth.",
    "The subject name being 'Assembly 1' is not enough to classify the review as a full assembly. If the CAD hierarchy and prompt point to one compact mechanism module, plan a focused mechanism review.",
    ...
    "If one reviewer lens is enough to answer the requested scope, enable only that reviewer. Extra reviewers are expensive and often turn checks into duplicate warnings.",
    ...
    "Do not treat Mechbase results as authoritative by default. If you fetch an image artifact, verify that it visibly reflects the precedent you intend to use before telling the user it is relevant.",
...
export function buildDeepDivePrompt(input: {
  ...
  return [
    ...
    "If Mechbase precedent from earlier passes is relevant, preserve it with source PDF/page or artifact URL. If you need to inspect a Mechbase image artifact, fetch only the single most relevant candidate and use it only after verifying the image matches the finding.",
...
export function buildSynthesisPrompt(input: {
  ...
  return [
    ...
    "Do not call request_user_input, ask_question, or any other hidden-user-question tool. If context is missing, preserve it in unresolvedQuestions while still giving the best evidence-grounded review possible.",
    ...
    "Do not promote duplicate concerns from different reviewers into separate action items. If two reviewers flag the same visible issue, merge them into one action item and preserve the strongest rationale.",
    "Do not promote a generic check into an action item unless it changes the design decision. For example, 'confirm belt ratio' is unresolved context unless the current ratio is visibly mismatched to the stated mechanism use.",
    "When reviewers provide visible tooth counts or calculated ratios, preserve the actual ratio judgment in the final review instead of replacing it with a generic belt-fit or packaging action.",
    "Preserve Mechbase precedent citations only when the reviewer or deep-dive report made a concrete relevance check. Do not include a Mechbase image or citation in the final review just because it was retrieved.",
    ...
    "Write the final review in plain language for FRC students. Avoid compressed jargon such as shorthand load-path phrases when a sentence about the visible plate, shaft, pulley, fastener, or tube would be clearer.",
    "Action item titles must name the physical part and practical change. Do not write titles like 'close the output load path'; write titles like 'support the output shaft close to the wheel or sprocket load'.",
    "",
    ...(input.reviewPlan ? ["Mechanism plan:", JSON.stringify(input.reviewPlan, null, 2), ""] : []),
