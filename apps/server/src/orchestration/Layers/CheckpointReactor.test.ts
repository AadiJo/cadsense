...
function createGitRepository() {
  const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "cadsense-checkpoint-handler-"));
...
describe("CheckpointReactor", () => {
  ...
  async function createHarness(options?: {
    ...
    const ServerConfigLayer = ServerConfig.layerTest(process.cwd(), {
      prefix: "cadsense-checkpoint-reactor-test-",
    });
  ...
  it("continues processing runtime events after a single checkpoint runtime failure", async () => {
    const nonRepositorySessionCwd = fs.mkdtempSync(
      path.join(os.tmpdir(), "cadsense-checkpoint-runtime-non-repo-"),
    );
  ...
  it("executes provider revert and emits thread.reverted for checkpoint revert requests", async () => {
    ...
    expect(fs.readFileSync(path.join(harness.cwd, "README.md"), "utf8").trimEnd()).toBe("v2");
    expect(
