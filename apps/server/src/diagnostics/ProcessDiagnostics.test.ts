...
function mockQueryProcessStdout(): string {
  ...
  return [
    ` ${serverPid}     1 ${serverPid} Ss 0.0 1024 01:02.03 cadsense server`,
    ` 4242 ${serverPid} ${serverPid} S  1.5 2048 00:04 agent`,
  ].join("\n");
}

function mockSignalQueryProcessStdout(): string {
  if (process.platform === "win32") {
    return JSON.stringify([
      {
        ProcessId: serverPid,
      ...
      },
      {
        ...
        ParentProcessId: serverPid,
        Name: "ps",
        CommandLine: "ps -axo pid=,ppid=,pgid=,stat=,pcpu=,rss=,etime=,command=",
        Status: "Running",
        WorkingSetSize: 2048,
        PercentProcessorTime: 1.5,
      },
  ...
  return [
    ` ${serverPid}     1 ${serverPid} Ss 0.0 1024 01:02.03 cadsense server`,
    ` 4242 ${serverPid} ${serverPid} R  1.5 2048 00:00 ps -axo pid=,ppid=,pgid=,stat=,pcpu=,rss=,etime=,command=`,
  ].join("\n");
}
  ...
  it.effect("queries processes through the ChildProcessSpawner service", () =>
    Effect.gen(function* () {
      ...
      if (process.platform === "win32") {
        ...
        expect(commands[0]?.args[0]).toBe("-NoProfile");
      } else {
    ...
    Effect.gen(function* () {
      const spawnerLayer = Layer.succeed(
        ...
        ChildProcessSpawner.make(() =>
          Effect.succeed(
            mockHandle({
              stdout: mockSignalQueryProcessStdout(),
            }),
          ),
        ),
      );
      ...
      expect(result).toEqual({
        ...
        signal: "SIGINT",
        signaled: false,
        message: Option.some("Process 4242 is not a live descendant of the CadSense server."),
      });
    }),
  );
});
