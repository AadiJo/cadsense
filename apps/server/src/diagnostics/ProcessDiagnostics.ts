...
>()("cadsense/diagnostics/ProcessDiagnostics") {}
...
> {
  const command = [
    "$processes = Get-CimInstance Win32_Process | Select-Object ProcessId, ParentProcessId, Name, CommandLine, Status, WorkingSetSize, @{Name='PercentProcessorTime';Expression={0}};",
...
function assertDescendantPid(
  ...
  if (pid === process.pid) {
    return Effect.fail(
      toProcessDiagnosticsError("Refusing to signal the CadSense server process."),
    );
  }
      ...
      return descendant
        ...
        : Effect.fail(
            toProcessDiagnosticsError(
              `Process ${pid} is not a live descendant of the CadSense server.`,
            ),
          );
    }),
  );
...
export const make = Effect.fn("makeProcessDiagnostics")(function* () {
  ...
  const read: ProcessDiagnosticsShape["read"] = Effect.matchEffect(
    Effect.gen(function* () {
        ...
        DateTime.now.pipe(
          Effect.map((readAt) =>
            makeResult({
              ...
              readAt,
              error:
                error instanceof Error ? error.message : "Failed to query process diagnostics.",
            }),
          ),
        ),
  ...
  const signal: ProcessDiagnosticsShape["signal"] = Effect.fn("ProcessDiagnostics.signal")(
    function* (input) {
      return yield* Effect.matchEffect(
        Effect.gen(function* () {
          yield* assertDescendantPid(input.pid).pipe(
            Effect.provideService(ChildProcessSpawner.ChildProcessSpawner, spawner),
          );
        ...
        }),
        {
          onFailure: (error) =>
            Effect.succeed({
              pid: input.pid,
              signal: input.signal,
              signaled: false as const,
              message: Option.some(
                error instanceof Error ? error.message : "Failed to signal the target process.",
              ),
            }),
          onSuccess: Effect.succeed,
        },
