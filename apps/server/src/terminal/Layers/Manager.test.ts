...
> =>
  Effect.flatMap(Effect.service(FileSystem.FileSystem), (fs) =>
    Effect.gen(function* () {
      ...
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "cadsense-terminal-" });
  ...
  it.effect("filters app runtime env variables from terminal sessions", () =>
    Effect.gen(function* () {
      ...
      try {
        ...
        expect(spawnInput.env.CADSENSE_PORT).toBeUndefined();
  ...
  it.effect("injects runtime env overrides into spawned terminals", () =>
    Effect.gen(function* () {
      ...
      yield* manager.open(
        openInput({
          env: {
            CADSENSE_PROJECT_ROOT: "/repo",
            CADSENSE_WORKTREE_PATH: "/repo/worktree-a",
            CUSTOM_FLAG: "1",
          },
        }),
      );
      ...
      assert.equal(spawnInput.env.CADSENSE_PROJECT_ROOT, "/repo");
      assert.equal(spawnInput.env.CADSENSE_WORKTREE_PATH, "/repo/worktree-a");
