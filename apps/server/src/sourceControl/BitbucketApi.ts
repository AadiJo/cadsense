...
import { sanitizeBranchFragment } from "@cadsense/shared/git";
import { detectSourceControlProviderFromRemoteUrl } from "@cadsense/shared/sourceControl";
...
const BitbucketApiEnvConfig = Config.all({
  baseUrl: Config.string("CADSENSE_BITBUCKET_API_BASE_URL").pipe(
    Config.withDefault(DEFAULT_API_BASE_URL),
  ),
  accessToken: Config.string("CADSENSE_BITBUCKET_ACCESS_TOKEN").pipe(Config.option),
  email: Config.string("CADSENSE_BITBUCKET_EMAIL").pipe(Config.option),
  apiToken: Config.string("CADSENSE_BITBUCKET_API_TOKEN").pipe(Config.option),
});
...
export class BitbucketApi extends Context.Service<BitbucketApi, BitbucketApiShape>()(
  "cadsense/source-control/BitbucketApi",
) {}
...
function checkoutBranchName(input: {
  ...
  return `cadsense/pr-${input.pullRequestId}/${sanitizeBranchFragment(input.headBranch)}`;
}
...
function authFromConfig(
  ...
  return {
    ...
    host: Option.some("bitbucket.org"),
    detail: Option.some(
      "Set CADSENSE_BITBUCKET_EMAIL and CADSENSE_BITBUCKET_API_TOKEN, or CADSENSE_BITBUCKET_ACCESS_TOKEN.",
    ),
  };
}
