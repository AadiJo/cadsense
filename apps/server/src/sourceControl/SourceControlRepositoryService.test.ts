...
import { GitCommandError, type SourceControlProviderError } from "@cadsense/contracts";
...
const CLONE_URLS = {
  ...
  url: "https://github.com/octocat/cadsense",
  sshUrl: "git@github.com:octocat/cadsense.git",
};
...
function makeLayer(input: {
  ...
  return SourceControlRepositoryService.layer.pipe(
    ...
    Layer.provide(
      ServerConfig.layerTest(process.cwd(), { prefix: "cadsense-source-control-repos-" }),
    ),
...
it.effect("looks up repositories through the requested provider without search", () => {
  ...
  return Effect.gen(function* () {
    ...
    assert.deepStrictEqual(calls, [{ cwd: "/workspace", repository: "octocat/cadsense" }]);
...
it.effect("clones a looked-up repository into the requested destination", () =>
  Effect.gen(function* () {
    ...
    const parent = yield* fs.makeTempDirectoryScoped({
      prefix: "cadsense-source-control-clone-parent-",
    });
    const path = yield* Path.Path;
    const destinationPath = path.join(parent, "cadsense");
    ...
    yield* Effect.gen(function* () {
      ...
      assert.deepStrictEqual(cloneCalls, [
        {
          cwd: parent,
          args: ["clone", CLONE_URLS.url, "cadsense"],
        },
      ]);
    }).pipe(
      Effect.provide(
...
it.effect("publishes by creating the repository, adding a remote, and pushing upstream", () => {
  ...
  return Effect.gen(function* () {
    ...
    assert.deepStrictEqual(createCalls, [
      { cwd: "/workspace", repository: "octocat/cadsense", visibility: "private" },
    ]);
