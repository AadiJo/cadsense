...
it.layer(NodeServices.layer)("bin cli parsing", (it) => {
  ...
  it.effect("executes auth pairing subcommands and redacts secrets from list output", () =>
    Effect.gen(function* () {
      const baseDir = mkdtempSync(join(tmpdir(), "cadsense-cli-auth-pairing-test-"));
  ...
  it.effect("executes auth session subcommands and redacts secrets from list output", () =>
    Effect.gen(function* () {
      const baseDir = mkdtempSync(join(tmpdir(), "cadsense-cli-auth-session-test-"));
  ...
  it.effect("rejects invalid ttl values before running auth commands", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(error.commandPath, ["cadsense", "auth", "pairing", "create"]);
  ...
  it.effect("adds, renames, and removes projects offline through the orchestration engine", () =>
    Effect.gen(function* () {
      const baseDir = mkdtempSync(join(tmpdir(), "cadsense-cli-projects-offline-test-"));
      const workspaceRoot = mkdtempSync(join(tmpdir(), "cadsense-cli-projects-workspace-"));
  ...
  it.effect("routes project commands through a running server when runtime state is present", () =>
    Effect.gen(function* () {
      const baseDir = mkdtempSync(join(tmpdir(), "cadsense-cli-projects-live-test-"));
      const workspaceRoot = mkdtempSync(join(tmpdir(), "cadsense-cli-projects-live-workspace-"));
  ...
  it.effect("rejects dev-url on project commands", () =>
    Effect.gen(function* () {
      const workspaceRoot = mkdtempSync(
        join(tmpdir(), "cadsense-cli-projects-unknown-option-workspace-"),
      );
      ...
      assert.deepEqual(error.commandPath, ["cadsense", "project", "add"]);
