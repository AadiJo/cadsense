...
function isWindowsSymlinkCreationDenied(error: unknown): boolean {
  ...
  for (let depth = 0; depth < 8 && current !== undefined; depth += 1) {
    if (
      ...
      current !== null &&
      (current as { code?: string }).code === "EPERM" &&
      (current as { syscall?: string }).syscall === "symlink"
    ) {
      return true;
    }
    if (typeof current === "object" && current !== null) {
      const rec = current as { cause?: unknown; reason?: { cause?: unknown } };
      current = rec.cause ?? rec.reason?.cause;
    } else {
      break;
    }
  }
  const text = `${error}`;
  return (
    text.includes("symlink") && (text.includes("EPERM") || text.includes("operation not permitted"))
  );
}
...
it.layer(NodeServices.layer)("mock-update-server", (it) => {
  ...
  it.effect("rejects symlinked files that escape the configured root", () =>
    Effect.gen(function* () {
      ...
      const symlinkOk = yield* Effect.matchEffect(fileSystem.symlink(outsideFile, symlinkPath), {
        onFailure: (error) =>
          isWindowsSymlinkCreationDenied(error) ? Effect.succeed(false) : Effect.fail(error),
