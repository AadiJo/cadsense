  ...
  describe("resolveOffset", () => {
    ...
    it.effect("throws for negative port offset", () =>
      Effect.gen(function* () {
        ...
        assert.ok(error.includes("Invalid CADSENSE_PORT_OFFSET"));
      }),
    );
  });

  describe("createDevRunnerEnv", () => {
    it.effect("defaults CADSENSE_HOME to ~/.cadsense when not provided", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.CADSENSE_HOME, path.resolve(NodeOS.homedir(), ".cadsense"));
      }),
    );
    ...
    it.effect("supports explicit typed overrides", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.CADSENSE_HOST, "0.0.0.0");
    ...
    it.effect("does not force websocket logging on in dev mode when unset", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.CADSENSE_MODE, "web");
    ...
    it.effect("pins desktop dev to a stable backend port and websocket url", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.CADSENSE_MODE, undefined);
        assert.equal(env.CADSENSE_NO_BROWSER, undefined);
        assert.equal(env.CADSENSE_HOST, undefined);
