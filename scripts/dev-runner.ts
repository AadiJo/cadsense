...
const MODE_ARGS = {
  ...
  "dev:server": ["run", "dev", "--filter=cadsense"],
  "dev:web": ["run", "dev", "--filter=@cadsense/web"],
  "dev:desktop": [
    "run",
    "dev",
...
export function resolveOffset(config: {
  ...
  if (config.portOffset !== undefined) {
    if (config.portOffset < 0) {
      throw new Error(`Invalid CADSENSE_PORT_OFFSET: ${config.portOffset}`);
    }
    return {
  ...
  if (/^\d+$/.test(seed)) {
    return { offset: Number(seed), source: `numeric CADSENSE_DEV_INSTANCE=${seed}` };
  }

  const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
  return { offset, source: `hashed CADSENSE_DEV_INSTANCE=${seed}` };
}
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    const resolvedBaseDir = yield* resolveBaseDir(cadsenseHome);
    ...
    if (!isDesktopMode) {
      ...
      delete output.CADSENSE_MODE;
      delete output.CADSENSE_NO_BROWSER;
      delete output.CADSENSE_HOST;
    }

    if (!isDesktopMode && host !== undefined) {
      output.CADSENSE_HOST = host;
    }

    if (!isDesktopMode && noBrowser !== undefined) {
      output.CADSENSE_NO_BROWSER = noBrowser ? "1" : "0";
    ...
    if (mode === "dev") {
      output.CADSENSE_MODE = "web";
      delete output.CADSENSE_DESKTOP_WS_URL;
    }
  ...
  return Effect.gen(function* () {
    const { portOffset, devInstance } = yield* OffsetConfig.asEffect().pipe(
      Effect.mapError(
        (cause) =>
          new DevRunnerError({
            message: "Failed to read CADSENSE_PORT_OFFSET/CADSENSE_DEV_INSTANCE configuration.",
            cause,
          }),
      ),
    );
    ...
    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.CADSENSE_PORT)} webPort=${String(env.PORT)} baseDir=${String(env.CADSENSE_HOME)}`,
    );
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  cadsenseHome: Flag.string("home-dir").pipe(
    Flag.withDescription("Base directory for all CadSense data (equivalent to CADSENSE_HOME)."),
    Flag.withFallbackConfig(optionalStringConfig("CADSENSE_HOME")),
  ),
  noBrowser: Flag.boolean("no-browser").pipe(
    Flag.withDescription("Browser auto-open toggle (equivalent to CADSENSE_NO_BROWSER)."),
    Flag.withFallbackConfig(optionalBooleanConfig("CADSENSE_NO_BROWSER")),
  ),
  ...
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to CADSENSE_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("CADSENSE_HOST")),
  ),
