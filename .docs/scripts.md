...
- Dev commands default `CADSENSE_STATE_DIR` to `~/.cadsense/dev` to keep dev state isolated from desktop/prod state.
- Override server CLI-equivalent flags from root dev commands with `--`, for example:
  `bun run dev -- --base-dir ~/.cadsense-2`
...
- Desktop production windows load the bundled UI from `cadsense://app/index.html` (not a `127.0.0.1` document URL).
- Desktop packaging includes `apps/server/dist` (the `cadsense` backend) and starts it on loopback with an auth token for WebSocket/API traffic.
...
Set `CADSENSE_DEV_INSTANCE` to any value to deterministically shift all dev ports together.

- Default ports: server `3773`, web `5733`
- Shifted ports: `base + offset` (offset is hashed from `CADSENSE_DEV_INSTANCE`)
- Example: `CADSENSE_DEV_INSTANCE=branch-a bun run dev:desktop`

If you want full control instead of hashing, set `CADSENSE_PORT_OFFSET` to a numeric offset.
