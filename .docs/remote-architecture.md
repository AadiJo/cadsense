...
This document describes the target architecture for first-class remote environments in CadSense.
...
- Keep the CadSense server as the execution boundary.
...
CadSense already has a clean runtime boundary: the client talks to a CadSense server over HTTP/WebSocket, and the server owns orchestration, providers, terminals, git, and filesystem operations.
...
└───────────────┬──────────────────────────────┘
                │
                │ connects to one CadSense server
                │
┌───────────────▼──────────────────────────────┐
│ Execution environment = one CadSense server       │
│                                              │
...
The important decision is that remoteness is expressed at the environment connection layer, not by splitting the CadSense runtime itself.

## Domain model

### ExecutionEnvironment

An `ExecutionEnvironment` is one running CadSense server instance.
...
How does the client speak WebSocket to a CadSense server?
...
- best fit for hosted or self-managed remote CadSense deployments
...
For CadSense, tunnels are best modeled as another `AccessEndpoint`, not as a different kind of environment.
...
Tailscale-backed access sits here architecturally even though the current implementation is endpoint discovery rather than a CadSense-managed tunnel. It contributes private-network endpoints and lets the existing HTTP/WebSocket client path do the actual connection.
...
- reach a machine
- probe it
- launch or reuse a remote CadSense server
...
How does a CadSense server come to exist on the target machine?
...
The user or operator already runs CadSense on the target machine, and the client connects through a direct or tunneled WebSocket endpoint.
...
What should be different in CadSense:
...
The recommended CadSense flow is:

1. Desktop connects over SSH.
2. Desktop probes the remote machine and verifies CadSense availability.
3. Desktop launches or reuses a remote CadSense server.
...
This is the inverse of remote launch: a local CadSense server is already running, and the client publishes it through a tunnel.
...
- A manually hosted CadSense server might be reached through direct `wss`.
...
CadSense already supports a WebSocket auth token on the server. That should become a first-class part of environment access rather than remaining an incidental query parameter convention.
...
CadSense should not copy that part.

CadSense already has the right runtime boundary:

- one CadSense server per environment
- ordinary HTTP/WebSocket between client and environment

So CadSense should borrow Zed's launch discipline, not its transport protocol.
