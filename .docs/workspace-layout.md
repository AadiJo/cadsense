...
- `/apps/desktop`: Electron shell. Spawns a desktop-scoped `cadsense` backend process and loads the shared web app.
...
- `/packages/shared`: Shared runtime utilities consumed by both server and web. Uses explicit subpath exports (e.g. `@cadsense/shared/git`, `@cadsense/shared/DrainableWorker`) — no barrel index.
